...
SERVICES: tuple[SOTService, ...] = (
    ...
    SOTService(
        ...
        depends_on=("integration.backoffice_adapter", "operations.work_orders"),
        notes=(
            ...
            "network retries safe. A separate typed manager approval owns the "
            "local financial decision and stages the idempotent ERP delivery "
            "intent. Submission never sends an unapproved expense. The vendor "
            "picker is read-only and only projects active vendor labels for "
            "field expense entry."
        ),
        contract=ServiceContract(
            concerns=(
            ...
            transaction=TransactionContract(
                mode=TransactionMode.OWNER_MANAGED,
                boundary=(
                    "Submission and work-order activity marking complete in one "
                    "owner transaction without ERP delivery. Manager approval "
                    "locks the request and, after cutover, must stage its ERP "
                    "outbox intent in the same owner transaction. The vendor "
                    "picker performs a "
