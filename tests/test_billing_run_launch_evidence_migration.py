...
def test_billing_run_evidence_is_the_single_migration_head() -> None:
    ...
    # Single linear head: Quote acceptance conversion (458) sits on reviewed
    # subledger openings (457), which sits on the ONT WAN service intent owner
    # (456), which sits on CPE RF signal columns (455), which sits on
    ...
    # customer WHT (419).
    assert script.get_heads() == ["458_quote_acceptance_sales_conversion"]
    assert (
        script.get_revision("458_quote_acceptance_sales_conversion").down_revision
        == "457_customer_subledger_opening_positions"
    )
    assert (
