...
def test_immediate_delivery_uses_exact_typed_outbox_identity():
    ...
    assert "notification_id: UUID" in ROUTE_SOURCE
    assert "notification_id=typed_notification_id" in TASK_SOURCE
    assert "outcome.notification_id" in ROUTE_SOURCE
    assert "background_tasks.add_task(" in ROUTE_SOURCE
    assert "deliver_notification.apply_async" in ROUTE_SOURCE
    assert "retry=False" in ROUTE_SOURCE
