...
| `ui.network_map_projection` | vendor route-planning plant projection | `resolver` | canonical network inventory and geometry ← `network.identity`<br>validated fiber route geometry ← `network.fiber_topology` | `read_only` | `complete` | network operations UI | `docs/designs/NETWORK_OPERATIONS_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_vendor_route_revision_authoring.py`<br>`tests/architecture/test_network_map_projection_boundary.py` |
