...
7. `ui.action_readiness_contracts` owns the transport-neutral `ActionReadiness`
   verdict for a gated action: state, actionable blockers (each attributed to
   a real, decision-making owner), next actions, and optional correlation
   metadata. It is not a workflow engine — nothing in the contract decides,
   persists, or executes anything; the owning domain service still decides
   eligibility, and `ActionReadiness` only changes the shape that decision is
   handed to a caller in. See `docs/designs/ACTION_READINESS_CONTRACT.md`.
...
The temporary live-chat exception accepted 2026-07-27 (ADR 0006) is RETIRED as
of 2026-08-30. `communications.team_inbox_widget` is the sole live-chat
authority for customer portal, reseller portal and the public fiber site. The
`comms.chat_session_authority` selector, the `crm.chat_session.v1` capability,
the CRM broker and the inbound CRM chat receiver are removed, not disabled: a
live-chat surface with two possible writers loses operator visibility the
moment they disagree, so the concept of a selectable chat authority no longer
exists in the codebase. `tests/architecture/test_single_chat_authority.py`
fails the build if a second broker destination, a chat-authority setting or an
external chat-transport capability reappears.
...
persistence-like mutation must name a declared owner. The 216 existing
...
identity, manually authored Lead- or customer-backed Quote, conditional
accepted-Quote account conversion, SalesOrder, configured
Project/Tasks/WorkOrders, vendor
implementation, ServiceOrder, Subscription activation, CX acceptance, and
later support history.

`sales.quote_authoring` owns the atomic Lead- or customer-backed Draft/Sent
admin creation boundary. Staff select exactly one lazy typeahead result. The
owner validates the authenticated staff principal, locks the exact eligible
Lead or active Subscriber, stores `lead_id` for an opportunity or
`subscriber_id` directly for an existing customer, batch-validates line
references and configured tax,
...
`sales.quote_delivery` owns the idempotent Send Email command. It resolves a
Lead-backed recipient through `Quote -> Lead -> Party` active contact points
and a customer-backed recipient from the selected active Subscriber email,
without requiring a Party binding. It records either the contact-point
reference or masked direct-recipient evidence and reuses the exact branded PDF.
For Subscriber-backed Quotes it consumes the typed
...
identity. `sales.quote_payment_review` owns the reviewer, decision time,
revision, and fingerprint of the exact commercial snapshot. The payment
projection resolves exact Subscriber scope, Quote lifecycle/expiry, current
snapshot approval, paid state, authoritative deposit, and Paystack capability
...
Quote and its selected subject. A Lead-backed Quote commits Lead Won and exact
Subscriber conversion; a customer-backed Quote validates and uses its existing
active Subscriber without a Lead or Party-binding prerequisite. Both paths
commit the copied order and lines, the Quote-selected Project Type, its
configured active template and
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `party.identity_reactivation` | reviewed quarantined Party identity reactivation | `command_writer` | attributable Party reactivation decision ← `party.identity_reactivation`<br>canonical Party identity ← `party.registry` | `owner_managed` | `native` | identity and authentication | `docs/PARTY_PRINCIPAL_CONTEXT_BINDING.md`<br>`docs/runbooks/PARTY_IDENTITY_REACTIVATION.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_party_identity_reactivation.py`<br>`tests/architecture/test_party_identity_reactivation_boundary.py` |
...
| `party.credential_authentication_projection` | installed authentication binding registry | `authoritative_record` | owner-declared authentication mechanism vocabulary ← `party.credential_authentication_projection`<br>installed verifier configuration evidence ← `party.credential_authentication_projection` | `owner_managed` | `shadowing` | identity and authentication | `docs/PARTY_PRINCIPAL_CONTEXT_BINDING.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_authentication_binding_installation.py`<br>`tests/test_credential_party_binding.py`<br>`tests/test_credential_party_binding_migration.py`<br>`tests/architecture/test_credential_party_binding_boundary.py`<br>`tests/test_staff_party_credential_adoption.py` |
| `party.credential_authentication_projection` | credential Party authentication projection | `projection_writer` | legacy credential principal reference ← `party.credential_authentication_projection`<br>reviewed Person Party binding ← `party.registry`<br>declared installed authentication binding ← `party.credential_authentication_projection`<br>operator tenant identity ← `tenancy.operator_tenant`<br>typed credential projection command evidence ← `party.credential_authentication_projection` | `owner_managed` | `shadowing` | identity and authentication | `docs/PARTY_PRINCIPAL_CONTEXT_BINDING.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_authentication_binding_installation.py`<br>`tests/test_credential_party_binding.py`<br>`tests/test_credential_party_binding_migration.py`<br>`tests/architecture/test_credential_party_binding_boundary.py`<br>`tests/test_staff_party_credential_adoption.py` |
| `party.credential_authentication_projection` | credential principal readiness and projection convergence report | `resolver` | legacy credential principal reference ← `party.credential_authentication_projection`<br>reviewed Person Party binding ← `party.registry`<br>declared installed authentication binding ← `party.credential_authentication_projection` | `owner_managed` | `shadowing` | identity and authentication | `docs/PARTY_PRINCIPAL_CONTEXT_BINDING.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_authentication_binding_installation.py`<br>`tests/test_credential_party_binding.py`<br>`tests/test_credential_party_binding_migration.py`<br>`tests/architecture/test_credential_party_binding_boundary.py`<br>`tests/test_staff_party_credential_adoption.py` |
...
| `customer.canonical_profile_patch` | typed transaction-neutral canonical Customer profile patches | `command_writer` | typed canonical Customer profile patch ← `customer.canonical_profile_patch`<br>locked canonical Customer account ← `customer.accounts` | `participant` | `native` | customer operations | `docs/designs/INBOX_CUSTOMER_COMPLETION_GATE.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_inbox_customer_completion.py`<br>`tests/architecture/test_inbox_customer_completion_boundary.py` |
...
| `customer.reseller_ticket_projection` | reseller-scoped native support ticket count and list projection | `resolver` | canonical reseller account scope ← `customer.identity_scope`<br>canonical native support ticket facts ← `support.ticket_lifecycle`<br>support-ticket semantic presentation ← `ui.status_presentation` | `read_only` | `complete` | customer support operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_reseller_portal_services.py`<br>`tests/test_api_reseller_self_scoped.py`<br>`tests/architecture/test_crm_web_retirement.py` |
...
| `customer.service_status` | customer-visible service health | `resolver` | canonical account state ← `customer.accounts`<br>operationally-current subscription cohort ← `access.subscription_lifecycle`<br>canonical billing profile ← `financial.billing_profile`<br>canonical customer financial position ← `customer.financial_position`<br>canonical service access decision ← `financial.access_resolution`<br>canonical grace decision ← `financial.grace_policy` | `read_only` | `complete` | customer operations | `docs/designs/PORTAL_ACCOUNT_SERVICE_HEALTH.md`<br>`docs/designs/CUSTOMER_SELF_SERVICE_LIFECYCLE.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_shared_policy_services.py`<br>`tests/test_service_status.py`<br>`tests/test_portal_account_health.py`<br>`tests/architecture/test_operational_subscription_cohort_boundary.py` |
| `customer.service_status` | customer financial action hints | `policy` | canonical billing profile ← `financial.billing_profile`<br>canonical customer financial position ← `customer.financial_position`<br>canonical service access decision ← `financial.access_resolution`<br>canonical grace decision ← `financial.grace_policy` | `read_only` | `complete` | customer operations | `docs/designs/PORTAL_ACCOUNT_SERVICE_HEALTH.md`<br>`docs/designs/CUSTOMER_SELF_SERVICE_LIFECYCLE.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_shared_policy_services.py`<br>`tests/test_service_status.py`<br>`tests/test_portal_account_health.py`<br>`tests/architecture/test_operational_subscription_cohort_boundary.py` |
| `customer.service_status` | payment-restores-service claims | `policy` | canonical customer financial position ← `customer.financial_position`<br>canonical service access decision ← `financial.access_resolution` | `read_only` | `complete` | customer operations | `docs/designs/PORTAL_ACCOUNT_SERVICE_HEALTH.md`<br>`docs/designs/CUSTOMER_SELF_SERVICE_LIFECYCLE.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_shared_policy_services.py`<br>`tests/test_service_status.py`<br>`tests/test_portal_account_health.py`<br>`tests/architecture/test_operational_subscription_cohort_boundary.py` |
...
| `customer.portal_location_commands` | customer portal service-address and pin update coordination | `application_coordinator` | authenticated customer location command evidence ← `customer.portal_location_commands`<br>canonical Subscriber account and service address ← `customer.accounts`<br>canonical service-address spatial projection ← `gis.spatial_sync` | `coordinator_managed` | `complete` | customer experience platform | `docs/designs/SUBSCRIBER_SERVICE_LOCATION_SOT.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/CODING_STANDARD.md`<br>`tests/test_customer_location_page.py`<br>`tests/architecture/test_adapter_transaction_ownership.py` |
| `customer.portal_location_commands` | customer portal location confirmation coordination | `application_coordinator` | authenticated customer location command evidence ← `customer.portal_location_commands`<br>canonical Subscriber account and service address ← `customer.accounts`<br>location capture and verification protocol ← `customer.location_capture` | `coordinator_managed` | `complete` | customer experience platform | `docs/designs/SUBSCRIBER_SERVICE_LOCATION_SOT.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/CODING_STANDARD.md`<br>`tests/test_customer_location_page.py`<br>`tests/architecture/test_adapter_transaction_ownership.py` |
| `customer.portal_location_commands` | customer portal location-prompt snooze coordination | `application_coordinator` | authenticated customer location command evidence ← `customer.portal_location_commands`<br>canonical Subscriber account and service address ← `customer.accounts`<br>location capture and verification protocol ← `customer.location_capture` | `coordinator_managed` | `complete` | customer experience platform | `docs/designs/SUBSCRIBER_SERVICE_LOCATION_SOT.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/CODING_STANDARD.md`<br>`tests/test_customer_location_page.py`<br>`tests/architecture/test_adapter_transaction_ownership.py` |
...
| `billing.receivable_projection` | billing receivable projection | `projection_writer` | incumbent invoice receivable state ← `financial.invoices`<br>incumbent settlement and allocation facts ← `financial.payments`<br>effective billing contract terms ← `billing.contracts`<br>subscription service scope ← `access.subscription_lifecycle`<br>authoritative non-standard billing treatment ← `financial.subscription_billing_treatments` | `owner_managed` | `shadowing` | billing and finance operations | `docs/adr/0007-end-to-end-billing-target-architecture.md`<br>`docs/designs/RECEIVABLE_PROJECTION_SHADOW.md`<br>`docs/runbooks/SUB_THIN_SHADOW.md`<br>`tests/test_receivable_projection.py`<br>`tests/test_receivable_parity_report.py`<br>`tests/integration/test_receivable_projection_monotonic.py`<br>`tests/architecture/test_receivable_projection_boundary.py` |
| `billing.receivable_projection` | receivable-shadow-01 cohort sealing and membership digest | `projection_writer` | incumbent invoice receivable state ← `financial.invoices` | `owner_managed` | `shadowing` | billing and finance operations | `docs/adr/0007-end-to-end-billing-target-architecture.md`<br>`docs/designs/RECEIVABLE_PROJECTION_SHADOW.md`<br>`docs/runbooks/SUB_THIN_SHADOW.md`<br>`tests/test_receivable_projection.py`<br>`tests/test_receivable_parity_report.py`<br>`tests/integration/test_receivable_projection_monotonic.py`<br>`tests/architecture/test_receivable_projection_boundary.py` |
| `billing.receivable_projection` | receivable projection version watermark | `projection_writer` | incumbent invoice receivable state ← `financial.invoices` | `owner_managed` | `shadowing` | billing and finance operations | `docs/adr/0007-end-to-end-billing-target-architecture.md`<br>`docs/designs/RECEIVABLE_PROJECTION_SHADOW.md`<br>`docs/runbooks/SUB_THIN_SHADOW.md`<br>`tests/test_receivable_projection.py`<br>`tests/test_receivable_parity_report.py`<br>`tests/integration/test_receivable_projection_monotonic.py`<br>`tests/architecture/test_receivable_projection_boundary.py` |
| `billing.receivable_projection` | receivable projection drift detection and idempotent repair | `reconciler` | incumbent invoice receivable state ← `financial.invoices`<br>incumbent settlement and allocation facts ← `financial.payments` | `owner_managed` | `shadowing` | billing and finance operations | `docs/adr/0007-end-to-end-billing-target-architecture.md`<br>`docs/designs/RECEIVABLE_PROJECTION_SHADOW.md`<br>`docs/runbooks/SUB_THIN_SHADOW.md`<br>`tests/test_receivable_projection.py`<br>`tests/test_receivable_parity_report.py`<br>`tests/integration/test_receivable_projection_monotonic.py`<br>`tests/architecture/test_receivable_projection_boundary.py` |
| `billing.receivable_projection` | receivable projection run evidence | `projection_writer` | incumbent invoice receivable state ← `financial.invoices`<br>shadow obligation counterparty ← `billing.obligations` | `owner_managed` | `shadowing` | billing and finance operations | `docs/adr/0007-end-to-end-billing-target-architecture.md`<br>`docs/designs/RECEIVABLE_PROJECTION_SHADOW.md`<br>`docs/runbooks/SUB_THIN_SHADOW.md`<br>`tests/test_receivable_projection.py`<br>`tests/test_receivable_parity_report.py`<br>`tests/integration/test_receivable_projection_monotonic.py`<br>`tests/architecture/test_receivable_projection_boundary.py` |
| `billing.receivable_projection` | receivable cutover readiness verdict | `resolver` | incumbent invoice receivable state ← `financial.invoices`<br>incumbent settlement and allocation facts ← `financial.payments`<br>effective billing contract terms ← `billing.contracts`<br>shadow obligation counterparty ← `billing.obligations`<br>subscription service scope ← `access.subscription_lifecycle`<br>authoritative non-standard billing treatment ← `financial.subscription_billing_treatments` | `owner_managed` | `shadowing` | billing and finance operations | `docs/adr/0007-end-to-end-billing-target-architecture.md`<br>`docs/designs/RECEIVABLE_PROJECTION_SHADOW.md`<br>`docs/runbooks/SUB_THIN_SHADOW.md`<br>`tests/test_receivable_projection.py`<br>`tests/test_receivable_parity_report.py`<br>`tests/integration/test_receivable_projection_monotonic.py`<br>`tests/architecture/test_receivable_projection_boundary.py` |
...
| `financial.customer_subledger_opening_positions` | reviewed customer-subledger opening-position correction | `command_writer` | reviewed opening correction command ← `auth.permission_gate`<br>recorded customer postings ← `financial.customer_subledger`<br>canonical customer account ← `customer.accounts` | `owner_managed` | `cutover_ready` | billing and finance operations | `docs/adr/0007-end-to-end-billing-target-architecture.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/runbooks/REVIEWED_MIGRATED_PREPAID_OPENING_REPAIR.md`<br>`tests/test_subledger_opening_positions.py`<br>`tests/architecture/test_customer_subledger_ownership.py`<br>`tests/architecture/test_billing_target_architecture.py` |
...
| `collections.module_shadow_parity` | postpaid Collections module eligibility parity evidence | `resolver` | canonical Sub invoice facts ← `financial.invoices`<br>incumbent Sub postpaid candidate rule ← `financial.dunning`<br>public Collections receivable blocker contract ← `collections.module_shadow_parity` | `read_only` | `shadowing` | billing operations | `docs/PLATFORM_ADOPTION_LEDGER.md`<br>`docs/adr/0011-module-lineage-composition.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_collections_module_shadow.py`<br>`tests/integration/test_collections_module_shadow_parity.py`<br>`tests/architecture/test_commercial_module_composition.py` |
...
| `integration.dotmac_erp_invoice_sync_projection` | versioned ERP invoice accounting sync projection | `resolver` | canonical invoice accounting source documents ← `financial.invoices`<br>issued invoice tax-rate snapshots ← `financial.tax_configuration` | `read_only` | `shadowing` | finance and platform operations | `docs/designs/ERP_INVOICE_ACCOUNTING_SYNC_V2.md`<br>`docs/designs/TAX_FACT_ADAPTER_AND_CLASSIFICATION_BACKFILL.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_invoice_accounting_sync_v2.py`<br>`tests/test_invoice_sync_digest.py`<br>`tests/architecture/test_sot_registry_integrity.py`<br>`tests/architecture/test_openapi_contract_surface.py` |
...
| `financial.topup_intents` | gateway reconciliation attempt and observation progress | `authoritative_record` | canonical top-up intent projection target ← `financial.topup_intents`<br>typed gateway reconciliation attempt ← `financial.payment_reconciliation`<br>typed gateway verification observation ← `external:payment_provider`<br>top-up intent transition protocol ← `financial.topup_intents` | `participant` | `complete` | finance operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`tests/test_payment_proofs.py`<br>`tests/test_direct_transfer_intents.py`<br>`tests/test_topup_intent_projection.py`<br>`tests/test_payment_webhook_settlement.py`<br>`tests/test_topup_intent_status.py`<br>`tests/architecture/test_topup_intent_ownership.py`<br>`tests/architecture/test_payment_intent_lifecycle_ownership.py` |
| `financial.payment_intent_management` | customer payment-intent history projection | `resolver` | canonical account payment intents ← `financial.topup_intents`<br>canonical linked payment-proof review evidence ← `financial.payment_proofs` | `owner_managed` | `complete` | finance operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_payment_intent_management.py` |
...
| `financial.payment_gateway_finance` | active gateway finance identity resolution | `resolver` | canonical payment provider identity ← `financial.payment_gateway_finance` | `participant` | `complete` | finance operations | `docs/designs/PAYMENT_GATEWAY_CONTROL_PLANE_SOT.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_web_integrations_payment_gateways.py`<br>`tests/test_payment_routing.py`<br>`tests/test_paystack_outside_window_recovery.py`<br>`tests/test_integrator_payment_provider_mapping.py`<br>`tests/architecture/test_sot_manifest_contracts.py`<br>`tests/architecture/test_integrator_settlement_boundary.py` |
...
| `financial.historical_invoice_tax_corrections` | reviewed historical invoice tax correction coordination | `application_coordinator` | reviewed historical tax correction command ← `financial.historical_invoice_tax_corrections`<br>canonical invoice correction documents ← `financial.invoices`<br>canonical selected payment funding ← `financial.account_credit_applications`<br>canonical customer VAT policy ← `financial.customer_tax_policies`<br>canonical tax-rate evidence ← `financial.tax_configuration` | `coordinator_managed` | `complete` | finance operations | `docs/designs/HISTORICAL_INVOICE_TAX_CORRECTION.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_historical_invoice_tax_corrections.py`<br>`tests/architecture/test_historical_invoice_tax_correction_boundary.py` |
| `financial.billing_tax_resolution` | compatibility subscription VAT treatment policy | `policy` | canonical subscription tax scope ← `access.subscription_lifecycle`<br>canonical customer VAT exemption policy ← `financial.customer_tax_policies`<br>active legacy tax-rate records ← `financial.tax_configuration`<br>catalog compatibility VAT fields ← `service_intent.catalog_policy`<br>configured compatibility VAT defaults ← `control.settings_spec` | `read_only` | `complete` | billing and finance operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/PLAN_FAMILY_ARCHITECTURE.md`<br>`tests/test_billing_tax_resolution.py`<br>`tests/test_billing_automation_services.py`<br>`tests/test_prepaid_threshold_resolver.py`<br>`tests/test_web_catalog_subscriptions.py`<br>`tests/architecture/test_billing_tax_resolution_boundary.py` |
| `financial.billing_tax_resolution` | bounded subscription VAT treatment resolution | `resolver` | canonical subscription tax scope ← `access.subscription_lifecycle`<br>canonical customer VAT exemption policy ← `financial.customer_tax_policies`<br>active legacy tax-rate records ← `financial.tax_configuration`<br>catalog compatibility VAT fields ← `service_intent.catalog_policy`<br>configured compatibility VAT defaults ← `control.settings_spec` | `read_only` | `complete` | billing and finance operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/PLAN_FAMILY_ARCHITECTURE.md`<br>`tests/test_billing_tax_resolution.py`<br>`tests/test_billing_automation_services.py`<br>`tests/test_prepaid_threshold_resolver.py`<br>`tests/test_web_catalog_subscriptions.py`<br>`tests/architecture/test_billing_tax_resolution_boundary.py` |
| `financial.billing_tax_resolution` | recorded-percent active tax-rate identity resolution | `resolver` | active legacy tax-rate records ← `financial.tax_configuration` | `read_only` | `complete` | billing and finance operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/PLAN_FAMILY_ARCHITECTURE.md`<br>`tests/test_billing_tax_resolution.py`<br>`tests/test_billing_automation_services.py`<br>`tests/test_prepaid_threshold_resolver.py`<br>`tests/test_web_catalog_subscriptions.py`<br>`tests/architecture/test_billing_tax_resolution_boundary.py` |
...
| `financial.manual_payment_recording` | administrative manual-payment duplicate-risk preview | `policy` | administrative manual-payment command evidence ← `financial.manual_payment_recording`<br>canonical subscriber account ← `customer.accounts`<br>canonical payment documents ← `financial.payments`<br>canonical payment-proof records ← `financial.payment_proofs` | `coordinator_managed` | `complete` | finance operations | `docs/designs/MANUAL_PAYMENT_DUPLICATE_CONTROL.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_manual_payment_recording.py`<br>`tests/test_billing_money_action_templates.py`<br>`tests/architecture/test_manual_payment_recording_boundary.py` |
| `financial.manual_payment_recording` | locked administrative manual-payment confirmation | `application_coordinator` | administrative manual-payment command evidence ← `financial.manual_payment_recording`<br>canonical subscriber account ← `customer.accounts`<br>canonical payment documents ← `financial.payments`<br>canonical payment-proof records ← `financial.payment_proofs`<br>canonical payment creation participant protocol ← `financial.payments`<br>canonical audit protocol ← `observability.audit_log` | `coordinator_managed` | `complete` | finance operations | `docs/designs/MANUAL_PAYMENT_DUPLICATE_CONTROL.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_manual_payment_recording.py`<br>`tests/test_billing_money_action_templates.py`<br>`tests/architecture/test_manual_payment_recording_boundary.py` |
...
| `financial.prepaid_draft_reconciliation` | historical paid prepaid invoice identity and coverage repair | `reconciler` | reviewed reconciliation command ← `financial.prepaid_draft_reconciliation`<br>reviewed historical paid-invoice repair command ← `auth.permission_gate`<br>canonical paid prepaid document gap ← `financial.invoices`<br>canonical prepaid subscription contract ← `access.subscription_lifecycle`<br>canonical paid invoice allocation evidence ← `financial.payments`<br>canonical funded service entitlement ← `financial.prepaid_service_renewals`<br>canonical direct-renewal debit ← `financial.prepaid_service_renewals`<br>canonical settlement business calendar ← `financial.prepaid_service_renewals`<br>financial access restoration protocol ← `financial.dunning` | `owner_managed` | `cut_over` | billing operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/FINANCIAL_ACCESS_ENFORCEMENT.md`<br>`docs/designs/PREPAID_DRAFT_RECONCILIATION.md`<br>`tests/test_prepaid_draft_reconciliation.py`<br>`tests/test_opening_settlement_correction.py`<br>`tests/test_web_prepaid_draft_reconciliation.py`<br>`tests/test_prepaid_service_renewals.py`<br>`tests/test_subscription_lifecycle_commands.py`<br>`tests/integration/test_prepaid_draft_reconciliation_concurrency.py`<br>`tests/architecture/test_prepaid_draft_reconciliation_ownership.py` |
| `financial.prepaid_draft_reconciliation` | reviewed paid prepaid invoice coverage correction | `reconciler` | reviewed reconciliation command ← `financial.prepaid_draft_reconciliation`<br>canonical paid prepaid coverage document ← `financial.invoices`<br>canonical prepaid subscription contract ← `access.subscription_lifecycle`<br>canonical paid invoice allocation evidence ← `financial.payments`<br>canonical funded service entitlement ← `financial.prepaid_service_renewals` | `owner_managed` | `cut_over` | billing operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/FINANCIAL_ACCESS_ENFORCEMENT.md`<br>`docs/designs/PREPAID_DRAFT_RECONCILIATION.md`<br>`tests/test_prepaid_draft_reconciliation.py`<br>`tests/test_opening_settlement_correction.py`<br>`tests/test_web_prepaid_draft_reconciliation.py`<br>`tests/test_prepaid_service_renewals.py`<br>`tests/test_subscription_lifecycle_commands.py`<br>`tests/integration/test_prepaid_draft_reconciliation_concurrency.py`<br>`tests/architecture/test_prepaid_draft_reconciliation_ownership.py` |
...
| `financial.prepaid_service_renewals` | prepaid service renewal execution | `command_writer` | prepaid subscription and renewal terms ← `billing.contracts`<br>effective compatibility tax treatment ← `financial.billing_tax_resolution`<br>settled payment evidence ← `financial.payments`<br>verified customer funding position ← `financial.prepaid_funding_reconstruction`<br>funded service entitlement evidence ← `financial.prepaid_service_renewals`<br>invoice and payment participant protocols ← `financial.invoices` | `owner_managed` | `cut_over` | billing and finance operations | `docs/adr/0007-end-to-end-billing-target-architecture.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/FINANCIAL_ACCESS_ENFORCEMENT.md`<br>`docs/designs/FUNDED_PREPAID_RENEWAL_INVOICING.md`<br>`docs/runbooks/LEGACY_PREPAID_RENEWAL_TAX_INVOICE_CORRECTION.md`<br>`docs/runbooks/REVIEWED_MIGRATED_PREPAID_OPENING_REPAIR.md`<br>`tests/test_prepaid_service_renewals.py`<br>`tests/services/billing/test_payment_status_recompute.py`<br>`tests/test_subledger_forward_shadow.py`<br>`tests/architecture/test_prepaid_billing_anchor_ownership.py` |
...
| `financial.prepaid_service_renewals` | prepaid settlement service-period resolution | `resolver` | settled payment evidence ← `financial.payments`<br>prepaid subscription and renewal terms ← `billing.contracts`<br>funded service entitlement evidence ← `financial.prepaid_service_renewals`<br>applied service-extension coverage evidence ← `financial.service_extensions` | `owner_managed` | `cut_over` | billing and finance operations | `docs/adr/0007-end-to-end-billing-target-architecture.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/FINANCIAL_ACCESS_ENFORCEMENT.md`<br>`docs/designs/FUNDED_PREPAID_RENEWAL_INVOICING.md`<br>`docs/runbooks/LEGACY_PREPAID_RENEWAL_TAX_INVOICE_CORRECTION.md`<br>`docs/runbooks/REVIEWED_MIGRATED_PREPAID_OPENING_REPAIR.md`<br>`tests/test_prepaid_service_renewals.py`<br>`tests/services/billing/test_payment_status_recompute.py`<br>`tests/test_subledger_forward_shadow.py`<br>`tests/architecture/test_prepaid_billing_anchor_ownership.py` |
| `financial.prepaid_service_renewals` | locked and idempotent funded prepaid renewal invoice settlement | `command_writer` | verified customer funding position ← `financial.prepaid_funding_reconstruction`<br>prepaid subscription and renewal terms ← `billing.contracts`<br>invoice and payment participant protocols ← `financial.invoices` | `owner_managed` | `cut_over` | billing and finance operations | `docs/adr/0007-end-to-end-billing-target-architecture.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/FINANCIAL_ACCESS_ENFORCEMENT.md`<br>`docs/designs/FUNDED_PREPAID_RENEWAL_INVOICING.md`<br>`docs/runbooks/LEGACY_PREPAID_RENEWAL_TAX_INVOICE_CORRECTION.md`<br>`docs/runbooks/REVIEWED_MIGRATED_PREPAID_OPENING_REPAIR.md`<br>`tests/test_prepaid_service_renewals.py`<br>`tests/services/billing/test_payment_status_recompute.py`<br>`tests/test_subledger_forward_shadow.py`<br>`tests/architecture/test_prepaid_billing_anchor_ownership.py` |
| `financial.prepaid_service_renewals` | exact paid-invoice-to-entitlement evidence | `authoritative_record` | verified customer funding position ← `financial.prepaid_funding_reconstruction`<br>prepaid subscription and renewal terms ← `billing.contracts`<br>invoice and payment participant protocols ← `financial.invoices` | `owner_managed` | `cut_over` | billing and finance operations | `docs/adr/0007-end-to-end-billing-target-architecture.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/FINANCIAL_ACCESS_ENFORCEMENT.md`<br>`docs/designs/FUNDED_PREPAID_RENEWAL_INVOICING.md`<br>`docs/runbooks/LEGACY_PREPAID_RENEWAL_TAX_INVOICE_CORRECTION.md`<br>`docs/runbooks/REVIEWED_MIGRATED_PREPAID_OPENING_REPAIR.md`<br>`tests/test_prepaid_service_renewals.py`<br>`tests/services/billing/test_payment_status_recompute.py`<br>`tests/test_subledger_forward_shadow.py`<br>`tests/architecture/test_prepaid_billing_anchor_ownership.py` |
...
| `financial.prepaid_service_renewals` | reviewed legacy prepaid renewal tax-invoice correction | `reconciler` | prepaid subscription and renewal terms ← `billing.contracts`<br>effective compatibility tax treatment ← `financial.billing_tax_resolution`<br>verified customer funding position ← `financial.prepaid_funding_reconstruction`<br>funded service entitlement evidence ← `financial.prepaid_service_renewals`<br>exact legacy account-adjustment evidence ← `financial.account_adjustments`<br>invoice and payment participant protocols ← `financial.invoices` | `owner_managed` | `cut_over` | billing and finance operations | `docs/adr/0007-end-to-end-billing-target-architecture.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/FINANCIAL_ACCESS_ENFORCEMENT.md`<br>`docs/designs/FUNDED_PREPAID_RENEWAL_INVOICING.md`<br>`docs/runbooks/LEGACY_PREPAID_RENEWAL_TAX_INVOICE_CORRECTION.md`<br>`docs/runbooks/REVIEWED_MIGRATED_PREPAID_OPENING_REPAIR.md`<br>`tests/test_prepaid_service_renewals.py`<br>`tests/services/billing/test_payment_status_recompute.py`<br>`tests/test_subledger_forward_shadow.py`<br>`tests/architecture/test_prepaid_billing_anchor_ownership.py` |
...
| `financial.payment_reconciliation` | stranded top-up reconciliation | `application_coordinator` | canonical top-up reconciliation policy ← `control.settings_spec`<br>canonical reconcilable top-up intent ← `financial.topup_intents`<br>typed gateway reconciliation progress ← `financial.topup_intents`<br>external gateway verification observation ← `external:payment_provider`<br>canonical gateway observation lifecycle protocol ← `financial.topup_intents` | `coordinator_managed` | `complete` | finance operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`docs/runbooks/PAYSTACK_AUTOMATIC_POSTING.md`<br>`tests/test_payment_webhook_settlement.py`<br>`tests/test_reconcile_honours_invoice_intent.py`<br>`tests/test_paystack_outside_window_recovery.py`<br>`tests/test_reconcile_paystack_reference_cli.py`<br>`tests/architecture/test_payment_reconciliation_ownership.py`<br>`tests/architecture/test_payment_settlement_participants.py` |
| `financial.payment_reconciliation` | scheduled top-up reconciliation execution | `application_coordinator` | canonical top-up reconciliation policy ← `control.settings_spec`<br>canonical reconcilable top-up intent ← `financial.topup_intents`<br>typed gateway reconciliation progress ← `financial.topup_intents`<br>external gateway verification observation ← `external:payment_provider` | `coordinator_managed` | `complete` | finance operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`docs/runbooks/PAYSTACK_AUTOMATIC_POSTING.md`<br>`tests/test_payment_webhook_settlement.py`<br>`tests/test_reconcile_honours_invoice_intent.py`<br>`tests/test_paystack_outside_window_recovery.py`<br>`tests/test_reconcile_paystack_reference_cli.py`<br>`tests/architecture/test_payment_reconciliation_ownership.py`<br>`tests/architecture/test_payment_settlement_participants.py` |
| `financial.payment_reconciliation` | outside-window exact-reference recovery preview and classification | `resolver` | canonical top-up reconciliation policy ← `control.settings_spec`<br>canonical reconcilable top-up intent ← `financial.topup_intents`<br>typed gateway reconciliation progress ← `financial.topup_intents`<br>typed exact-reference recovery request ← `financial.payment_reconciliation`<br>enabled version-pinned reconciliation capability binding ← `integration.installations`<br>canonical payment provider identity ← `financial.payment_gateway_finance`<br>external gateway verification observation ← `external:payment_provider`<br>canonical payment replay evidence ← `financial.payments`<br>canonical provider-event replay evidence ← `financial.payment_provider_events`<br>canonical exact-reference recovery evidence ← `financial.payment_reconciliation` | `coordinator_managed` | `complete` | finance operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`docs/runbooks/PAYSTACK_AUTOMATIC_POSTING.md`<br>`tests/test_payment_webhook_settlement.py`<br>`tests/test_reconcile_honours_invoice_intent.py`<br>`tests/test_paystack_outside_window_recovery.py`<br>`tests/test_reconcile_paystack_reference_cli.py`<br>`tests/architecture/test_payment_reconciliation_ownership.py`<br>`tests/architecture/test_payment_settlement_participants.py` |
| `financial.payment_reconciliation` | finance-reviewed exact-reference outside-window top-up recovery | `application_coordinator` | canonical top-up reconciliation policy ← `control.settings_spec`<br>canonical reconcilable top-up intent ← `financial.topup_intents`<br>typed gateway reconciliation progress ← `financial.topup_intents`<br>finance-reviewed exact-reference recovery confirmation ← `financial.payment_reconciliation`<br>enabled version-pinned reconciliation capability binding ← `integration.installations`<br>canonical payment provider identity ← `financial.payment_gateway_finance`<br>external gateway verification observation ← `external:payment_provider`<br>canonical payment replay evidence ← `financial.payments`<br>canonical provider-event replay evidence ← `financial.payment_provider_events`<br>canonical exact-reference recovery evidence ← `financial.payment_reconciliation`<br>canonical account-credit deposit protocol ← `financial.account_credit_deposits`<br>canonical provider-event settlement protocol ← `financial.payment_provider_events`<br>canonical top-up completion protocol ← `financial.topup_intents`<br>canonical gateway observation lifecycle protocol ← `financial.topup_intents` | `coordinator_managed` | `complete` | finance operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`docs/runbooks/PAYSTACK_AUTOMATIC_POSTING.md`<br>`tests/test_payment_webhook_settlement.py`<br>`tests/test_reconcile_honours_invoice_intent.py`<br>`tests/test_paystack_outside_window_recovery.py`<br>`tests/test_reconcile_paystack_reference_cli.py`<br>`tests/architecture/test_payment_reconciliation_ownership.py`<br>`tests/architecture/test_payment_settlement_participants.py` |
| `financial.payment_reconciliation` | finance-reviewed exact-reference recovery evidence | `authoritative_record` | canonical reconcilable top-up intent ← `financial.topup_intents`<br>finance-reviewed exact-reference recovery confirmation ← `financial.payment_reconciliation`<br>external gateway verification observation ← `external:payment_provider`<br>canonical payment replay evidence ← `financial.payments`<br>canonical provider-event replay evidence ← `financial.payment_provider_events` | `coordinator_managed` | `complete` | finance operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`docs/runbooks/PAYSTACK_AUTOMATIC_POSTING.md`<br>`tests/test_payment_webhook_settlement.py`<br>`tests/test_reconcile_honours_invoice_intent.py`<br>`tests/test_paystack_outside_window_recovery.py`<br>`tests/test_reconcile_paystack_reference_cli.py`<br>`tests/architecture/test_payment_reconciliation_ownership.py`<br>`tests/architecture/test_payment_settlement_participants.py` |
| `financial.payment_reconciliation` | verified provider settlement then allocation orchestration | `application_coordinator` | canonical reconcilable top-up intent ← `financial.topup_intents`<br>external gateway verification observation ← `external:payment_provider`<br>canonical account-credit deposit protocol ← `financial.account_credit_deposits`<br>canonical provider-event settlement protocol ← `financial.payment_provider_events`<br>canonical top-up completion protocol ← `financial.topup_intents` | `coordinator_managed` | `complete` | finance operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`docs/runbooks/PAYSTACK_AUTOMATIC_POSTING.md`<br>`tests/test_payment_webhook_settlement.py`<br>`tests/test_reconcile_honours_invoice_intent.py`<br>`tests/test_paystack_outside_window_recovery.py`<br>`tests/test_reconcile_paystack_reference_cli.py`<br>`tests/architecture/test_payment_reconciliation_ownership.py`<br>`tests/architecture/test_payment_settlement_participants.py` |
| `financial.payment_reconciliation` | top-up reconciliation backlog, progress, and age projection | `resolver` | canonical top-up reconciliation policy ← `control.settings_spec`<br>canonical reconcilable top-up intent ← `financial.topup_intents`<br>typed gateway reconciliation progress ← `financial.topup_intents` | `coordinator_managed` | `complete` | finance operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`docs/runbooks/PAYSTACK_AUTOMATIC_POSTING.md`<br>`tests/test_payment_webhook_settlement.py`<br>`tests/test_reconcile_honours_invoice_intent.py`<br>`tests/test_paystack_outside_window_recovery.py`<br>`tests/test_reconcile_paystack_reference_cli.py`<br>`tests/architecture/test_payment_reconciliation_ownership.py`<br>`tests/architecture/test_payment_settlement_participants.py` |
...
| `network.infrastructure_catalogue` | native infrastructure selection and exact-reference resolution | `resolver` | native inventory identities ← `network.identity` | `read_only` | `complete` | network inventory and service delivery | `docs/designs/PROJECT_INFRASTRUCTURE_SCOPE.md`<br>`tests/test_project_infrastructure.py`<br>`tests/architecture/test_project_infrastructure_boundary.py` |
...
| `network.live_bandwidth_observations` | subscription-scoped live bandwidth observation projection | `resolver` | canonical subscription-to-NAS binding ← `access.subscription_lifecycle`<br>canonical NAS identity and transport configuration ← `network.identity`<br>native RouterOS bandwidth observations ← `external:routeros`<br>live bandwidth freshness policy ← `network.live_bandwidth_observations` | `read_only` | `complete` | network operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/OPERATIONAL_EVIDENCE_AND_RETRY.md`<br>`docs/runbooks/DATABASE_TRANSACTION_PRESSURE.md`<br>`tests/test_bandwidth_mikrotik_live_session.py`<br>`tests/test_customer_portal_gaps.py` |
| `network.live_bandwidth_observations` | direct RouterOS bandwidth diagnostic admission | `policy` | canonical subscription-to-NAS binding ← `access.subscription_lifecycle`<br>canonical NAS identity and transport configuration ← `network.identity`<br>direct diagnostic admission policy ← `network.live_bandwidth_observations` | `read_only` | `complete` | network operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/OPERATIONAL_EVIDENCE_AND_RETRY.md`<br>`docs/runbooks/DATABASE_TRANSACTION_PRESSURE.md`<br>`tests/test_bandwidth_mikrotik_live_session.py`<br>`tests/test_customer_portal_gaps.py` |
| `network.live_bandwidth_observations` | active-viewer bandwidth polling demand signal | `policy` | connected live viewer observation ← `external:connected_client`<br>live bandwidth freshness policy ← `network.live_bandwidth_observations` | `read_only` | `complete` | network operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/OPERATIONAL_EVIDENCE_AND_RETRY.md`<br>`docs/runbooks/DATABASE_TRANSACTION_PRESSURE.md`<br>`tests/test_bandwidth_mikrotik_live_session.py`<br>`tests/test_customer_portal_gaps.py` |
...
| `network.lldp_observations` | LLDP adjacency observation reconciliation | `observation_collector` | router inventory ← `network.identity`<br>neighbor replies ← `external:routeros`<br>existing adjacency ← `network.monitoring_inventory` | `owner_managed` | `complete` | network operations | `docs/designs/LLDP_POLL_TRANSACTION_LIFETIME.md`<br>`tests/services/topology/test_lldp_poller.py`<br>`tests/test_lldp_transaction_span.py`<br>`tests/architecture/test_lldp_transaction_boundary.py` |
...
| `network.cpe_assignment_drift` | read-only live-device/no-active-assignment review projection | `resolver` | canonical active ONT assignment ← `network.ont_assignment_identity`<br>fresh exact-subscription RADIUS session ← `network.radius_sessions`<br>recent GenieACS Inform ← `external:genieacs` | `read_only` | `native` | network operations | `docs/designs/ONT_UI_SERVICE_CONFIGURATION_SOT.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_cpe_assignment_drift.py`<br>`tests/test_network_explorer.py` |
...
| `network.outage_work_order_handoff` | shared-outage infrastructure ticket issuance | `application_coordinator` | locked outage incident and scope revision ← `network.outage_lifecycle`<br>outage-response routing team ← `operations.service_team_composition`<br>typed infrastructure ticket create input ← `network.outage_work_order_handoff` | `coordinator_managed` | `native` | network operations | `docs/designs/OUTAGE_WORK_ORDER_HANDOFF_SOT.md`<br>`docs/designs/NETWORK_OUTAGE_RESPONSE_LIFECYCLE.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/services/topology/test_outage_work_order_handoff_contract.py`<br>`tests/architecture/test_ticket_work_order_handoff_boundary.py` |
| `network.outage_work_order_handoff` | shared-outage work-order issuance eligibility | `application_coordinator` | locked outage incident and scope revision ← `network.outage_lifecycle`<br>canonical infrastructure ticket ← `support.ticket_lifecycle`<br>assigned team membership ← `support.ticket_lifecycle`<br>typed infrastructure work-order request ← `network.outage_work_order_handoff` | `coordinator_managed` | `native` | network operations | `docs/designs/OUTAGE_WORK_ORDER_HANDOFF_SOT.md`<br>`docs/designs/NETWORK_OUTAGE_RESPONSE_LIFECYCLE.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/services/topology/test_outage_work_order_handoff_contract.py`<br>`tests/architecture/test_ticket_work_order_handoff_boundary.py` |
| `network.outage_work_order_handoff` | outage-to-work-order provenance | `authoritative_record` | locked outage incident and scope revision ← `network.outage_lifecycle`<br>native work-order result ← `operations.work_order_commands` | `coordinator_managed` | `native` | network operations | `docs/designs/OUTAGE_WORK_ORDER_HANDOFF_SOT.md`<br>`docs/designs/NETWORK_OUTAGE_RESPONSE_LIFECYCLE.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/services/topology/test_outage_work_order_handoff_contract.py`<br>`tests/architecture/test_ticket_work_order_handoff_boundary.py` |
...
| `app_sessions.refresh` | concurrency-safe database authentication session renewal | `command_writer` | database authentication session state ← `app_sessions.refresh`<br>presented refresh credential and client binding ← `external:auth_client`<br>canonical staff session principal projection ← `party.staff_session_projection`<br>access-token signing policy ← `control.settings_spec`<br>held JWT signing secret ← `secrets.reference_store` | `owner_managed` | `complete` | identity and platform operations | `docs/designs/AUTH_SESSION_REFRESH_CONCURRENCY.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_auth_session_refresh.py`<br>`tests/integration/test_auth_session_refresh_concurrency.py`<br>`tests/js/session_refresh.test.js` |
...
| `communications.staff_notifications` | admin/staff notification creation | `command_writer` | validated staff notification request ← `auth.permission_gate`<br>validated staff assignment notification command ← `communications.staff_notifications`<br>canonical staff account identity ← `auth.staff_provisioning`<br>canonical internal notification row ← `communications.notification_service` | `participant` | `complete` | customer experience platform | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/NOTIFICATION_CHANNEL_POLICY.md`<br>`docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/designs/SUPPORT_TICKET_LIFECYCLE_SOT.md`<br>`tests/test_team_inbox_readiness_gate.py`<br>`tests/test_staff_assignment_notifications.py`<br>`tests/test_support_services.py`<br>`tests/architecture/test_sot_manifest_contracts.py` |
| `communications.staff_notifications` | permission-targeted staff notification audience resolution | `resolver` | validated staff notification request ← `auth.permission_gate`<br>validated staff assignment notification command ← `communications.staff_notifications`<br>canonical staff account identity ← `auth.staff_provisioning` | `participant` | `complete` | customer experience platform | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/NOTIFICATION_CHANNEL_POLICY.md`<br>`docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/designs/SUPPORT_TICKET_LIFECYCLE_SOT.md`<br>`tests/test_team_inbox_readiness_gate.py`<br>`tests/test_staff_assignment_notifications.py`<br>`tests/test_support_services.py`<br>`tests/architecture/test_sot_manifest_contracts.py` |
| `communications.staff_notifications` | staff review inbox materialization | `projection_writer` | canonical staff account identity ← `auth.staff_provisioning`<br>canonical internal notification row ← `communications.notification_service` | `participant` | `complete` | customer experience platform | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/NOTIFICATION_CHANNEL_POLICY.md`<br>`docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/designs/SUPPORT_TICKET_LIFECYCLE_SOT.md`<br>`tests/test_team_inbox_readiness_gate.py`<br>`tests/test_staff_assignment_notifications.py`<br>`tests/test_support_services.py`<br>`tests/architecture/test_sot_manifest_contracts.py` |
| `communications.staff_notification_read_state` | personal staff notification menu and unread count | `resolver` | canonical staff account identity ← `auth.staff_provisioning`<br>current personal staff inbox identity and target ← `communications.staff_notifications`<br>current personal staff notification read state ← `communications.staff_notification_read_state` | `owner_managed` | `cut_over` | customer experience platform | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_web_admin_notifications_service.py`<br>`tests/test_payment_proof_reviewer_notifications.py`<br>`tests/architecture/test_sot_manifest_contracts.py` |
| `communications.staff_notification_read_state` | personal staff notification open and legacy target repair | `command_writer` | typed personal notification open command ← `communications.staff_notification_read_state`<br>canonical staff account identity ← `auth.staff_provisioning`<br>current personal staff inbox identity and target ← `communications.staff_notifications`<br>current personal staff notification read state ← `communications.staff_notification_read_state`<br>canonical ticket assignment reference ← `support.ticket_lifecycle`<br>canonical project assignment reference ← `operations.project_lifecycle` | `owner_managed` | `cut_over` | customer experience platform | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_web_admin_notifications_service.py`<br>`tests/test_payment_proof_reviewer_notifications.py`<br>`tests/architecture/test_sot_manifest_contracts.py` |
| `communications.nextcloud_talk_staff` | staff-to-Nextcloud username mapping | `authoritative_record` | validated staff Talk command ← `auth.permission_gate`<br>canonical staff account identity ← `auth.staff_provisioning`<br>enabled Talk installation and binding ← `integration.installations`<br>current staff Talk state ← `communications.nextcloud_talk_staff` | `owner_managed` | `native` | customer experience platform | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/ERP_WORKFORCE_ACCOUNT_PROVISIONING.md`<br>`docs/designs/INTEGRATION_PLATFORM_SOT.md`<br>`docs/designs/NOTIFICATION_CHANNEL_POLICY.md`<br>`tests/test_api_staff_sync.py`<br>`tests/test_nextcloud_talk_staff_notifications.py`<br>`tests/architecture/test_sot_manifest_contracts.py`<br>`tests/architecture/test_adapter_transaction_ownership.py` |
| `communications.nextcloud_talk_staff` | staff direct-room token projection | `projection_writer` | current staff Talk state ← `communications.nextcloud_talk_staff`<br>version-pinned Talk operation outcome ← `integration.runtime` | `owner_managed` | `native` | customer experience platform | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/ERP_WORKFORCE_ACCOUNT_PROVISIONING.md`<br>`docs/designs/INTEGRATION_PLATFORM_SOT.md`<br>`docs/designs/NOTIFICATION_CHANNEL_POLICY.md`<br>`tests/test_api_staff_sync.py`<br>`tests/test_nextcloud_talk_staff_notifications.py`<br>`tests/architecture/test_sot_manifest_contracts.py`<br>`tests/architecture/test_adapter_transaction_ownership.py` |
| `communications.nextcloud_talk_staff` | Nextcloud Talk staff delivery admission and idempotency | `command_writer` | canonical staff account identity ← `auth.staff_provisioning`<br>current staff notification delivery row ← `communications.notification_service`<br>enabled Talk installation and binding ← `integration.installations` | `owner_managed` | `native` | customer experience platform | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/ERP_WORKFORCE_ACCOUNT_PROVISIONING.md`<br>`docs/designs/INTEGRATION_PLATFORM_SOT.md`<br>`docs/designs/NOTIFICATION_CHANNEL_POLICY.md`<br>`tests/test_api_staff_sync.py`<br>`tests/test_nextcloud_talk_staff_notifications.py`<br>`tests/architecture/test_sot_manifest_contracts.py`<br>`tests/architecture/test_adapter_transaction_ownership.py` |
| `communications.nextcloud_talk_staff` | Nextcloud Talk staff delivery retry and reconciliation policy | `reconciler` | current staff notification delivery row ← `communications.notification_service`<br>current staff Talk state ← `communications.nextcloud_talk_staff`<br>version-pinned Talk operation outcome ← `integration.runtime` | `owner_managed` | `native` | customer experience platform | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/ERP_WORKFORCE_ACCOUNT_PROVISIONING.md`<br>`docs/designs/INTEGRATION_PLATFORM_SOT.md`<br>`docs/designs/NOTIFICATION_CHANNEL_POLICY.md`<br>`tests/test_api_staff_sync.py`<br>`tests/test_nextcloud_talk_staff_notifications.py`<br>`tests/architecture/test_sot_manifest_contracts.py`<br>`tests/architecture/test_adapter_transaction_ownership.py` |
| `communications.team_inbox_participants` | conversation participant endpoint projection | `projection_writer` | stored conversation message headers ← `communications.team_inbox_threads`<br>owned mailbox register ← `communications.team_inbox_routing`<br>reviewed participant relationship command ← `auth.permission_gate` | `participant` | `complete` | customer experience platform | `docs/designs/INBOX_CONVERSATION_PARTICIPANTS.md`<br>`tests/test_team_inbox_participants.py`<br>`tests/architecture/test_team_inbox_boundaries.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py` |
| `communications.team_inbox_participants` | reviewed conversation participant relationship classification | `projection_writer` | stored conversation message headers ← `communications.team_inbox_threads`<br>owned mailbox register ← `communications.team_inbox_routing`<br>reviewed participant relationship command ← `auth.permission_gate` | `participant` | `complete` | customer experience platform | `docs/designs/INBOX_CONVERSATION_PARTICIPANTS.md`<br>`tests/test_team_inbox_participants.py`<br>`tests/architecture/test_team_inbox_boundaries.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py` |
...
| `communications.team_inbox_customer_completion_policy` | immutable Customer resolution-completion policy versions | `command_writer` | authorized Inbox settings decision ← `auth.permission_gate` | `owner_managed` | `complete` | customer experience platform | `docs/designs/INBOX_CUSTOMER_COMPLETION_GATE.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_inbox_customer_completion.py` |
| `communications.team_inbox_customer_completion` | Customer-only Inbox resolution readiness | `resolver` | snapshotted Customer completion policy ← `communications.team_inbox_customer_completion_policy`<br>conversation Customer or Lead identity ← `communications.team_inbox_threads`<br>canonical Lead relationship ← `communications.conversation_lead_relationships`<br>canonical Customer profile ← `customer.accounts`<br>canonical Party profile ← `party.registry` | `coordinator_managed` | `complete` | customer experience platform | `docs/designs/INBOX_CUSTOMER_COMPLETION_GATE.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_inbox_customer_completion.py`<br>`tests/architecture/test_inbox_customer_completion_boundary.py` |
| `communications.team_inbox_customer_completion` | canonical Inbox Customer profile completion coordination | `application_coordinator` | snapshotted Customer completion policy ← `communications.team_inbox_customer_completion_policy`<br>conversation Customer or Lead identity ← `communications.team_inbox_threads`<br>canonical Lead relationship ← `communications.conversation_lead_relationships`<br>canonical Customer profile ← `customer.accounts`<br>canonical Party profile ← `party.registry` | `coordinator_managed` | `complete` | customer experience platform | `docs/designs/INBOX_CUSTOMER_COMPLETION_GATE.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_inbox_customer_completion.py`<br>`tests/architecture/test_inbox_customer_completion_boundary.py` |
| `communications.team_inbox_threads` | conversation identity and threading | `resolver` | normalized inbound message fact ← `communications.team_inbox_observations`<br>active Customer completion policy ← `communications.team_inbox_customer_completion_policy` | `participant` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_sot_completion.py`<br>`tests/architecture/test_team_inbox_boundaries.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py` |
| `communications.team_inbox_threads` | authoritative conversation and message records | `authoritative_record` | normalized inbound message fact ← `communications.team_inbox_observations`<br>active Customer completion policy ← `communications.team_inbox_customer_completion_policy` | `participant` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_sot_completion.py`<br>`tests/architecture/test_team_inbox_boundaries.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py` |
| `communications.team_inbox_contact_resolution` | contact subscriber reseller and ticket association resolution | `resolver` | canonical party contact facts ← `party.registry`<br>customer identity scope ← `customer.identity_scope`<br>conversation contact route ← `communications.team_inbox_threads`<br>conversation participant relationship ← `communications.team_inbox_participants` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/runbooks/TEAM_INBOX_SUBSCRIBER_LINK_REPAIR.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/designs/INBOX_CONVERSATION_PARTICIPANTS.md`<br>`tests/test_team_inbox_contact_links.py`<br>`tests/test_team_inbox_admin_contact_links.py`<br>`tests/test_repair_team_inbox_subscriber_links.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py` |
...
| `communications.team_inbox_contact_resolution` | conversation-aware lazy Customer link-option projection | `resolver` | canonical party contact facts ← `party.registry`<br>customer identity scope ← `customer.identity_scope`<br>conversation contact route ← `communications.team_inbox_threads`<br>conversation participant relationship ← `communications.team_inbox_participants` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/runbooks/TEAM_INBOX_SUBSCRIBER_LINK_REPAIR.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/designs/INBOX_CONVERSATION_PARTICIPANTS.md`<br>`tests/test_team_inbox_contact_links.py`<br>`tests/test_team_inbox_admin_contact_links.py`<br>`tests/test_repair_team_inbox_subscriber_links.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py` |
| `communications.team_inbox_contact_resolution` | conversation-scoped representative customer association resolution | `resolver` | canonical party contact facts ← `party.registry`<br>customer identity scope ← `customer.identity_scope`<br>conversation contact route ← `communications.team_inbox_threads`<br>conversation participant relationship ← `communications.team_inbox_participants` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/runbooks/TEAM_INBOX_SUBSCRIBER_LINK_REPAIR.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/designs/INBOX_CONVERSATION_PARTICIPANTS.md`<br>`tests/test_team_inbox_contact_links.py`<br>`tests/test_team_inbox_admin_contact_links.py`<br>`tests/test_repair_team_inbox_subscriber_links.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py` |
| `communications.team_inbox_contact_resolution` | exact provider-scoped Party identity evidence | `resolver` | canonical party contact facts ← `party.registry`<br>customer identity scope ← `customer.identity_scope`<br>conversation contact route ← `communications.team_inbox_threads`<br>conversation participant relationship ← `communications.team_inbox_participants` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/runbooks/TEAM_INBOX_SUBSCRIBER_LINK_REPAIR.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/designs/INBOX_CONVERSATION_PARTICIPANTS.md`<br>`tests/test_team_inbox_contact_links.py`<br>`tests/test_team_inbox_admin_contact_links.py`<br>`tests/test_repair_team_inbox_subscriber_links.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py` |
| `communications.team_inbox_contact_resolution` | Inbox-origin Lead endpoint identity repair | `reconciler` | canonical party contact facts ← `party.registry`<br>customer identity scope ← `customer.identity_scope`<br>conversation contact route ← `communications.team_inbox_threads`<br>conversation participant relationship ← `communications.team_inbox_participants` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/runbooks/TEAM_INBOX_SUBSCRIBER_LINK_REPAIR.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/designs/INBOX_CONVERSATION_PARTICIPANTS.md`<br>`tests/test_team_inbox_contact_links.py`<br>`tests/test_team_inbox_admin_contact_links.py`<br>`tests/test_repair_team_inbox_subscriber_links.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py` |
| `communications.team_inbox_routing` | routing assignment and escalation policy | `policy` | conversation routing facts ← `communications.team_inbox_threads`<br>operational escalation policy ← `operations.sla_escalation`<br>operator authorization ← `auth.permission_gate`<br>validated AI intake destination metadata ← `ai.intake`<br>active AI conversation ownership ← `ai.intake`<br>successful staff session issuance ← `app_sessions.auth`<br>authenticated visible Inbox activity ← `app_sessions.auth`<br>active staff principal ← `auth.staff_provisioning`<br>agent capacity configuration ← `control.settings_spec`<br>WhatsApp service-window state ← `communications.team_inbox_reply_window` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_admin_inbox_slice4_workflows.py`<br>`tests/test_auth_flow.py`<br>`tests/test_team_inbox_assignment.py`<br>`tests/test_team_inbox_commands.py`<br>`tests/test_team_inbox_fifo_queue.py`<br>`tests/test_team_inbox_queue_notifications.py`<br>`tests/integration/test_team_inbox_queue_concurrency.py` |
...
| `communications.team_inbox_routing` | strict per-team FIFO head serialization | `policy` | conversation routing facts ← `communications.team_inbox_threads`<br>operational escalation policy ← `operations.sla_escalation`<br>operator authorization ← `auth.permission_gate`<br>validated AI intake destination metadata ← `ai.intake`<br>active AI conversation ownership ← `ai.intake`<br>successful staff session issuance ← `app_sessions.auth`<br>authenticated visible Inbox activity ← `app_sessions.auth`<br>active staff principal ← `auth.staff_provisioning`<br>agent capacity configuration ← `control.settings_spec`<br>WhatsApp service-window state ← `communications.team_inbox_reply_window` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_admin_inbox_slice4_workflows.py`<br>`tests/test_auth_flow.py`<br>`tests/test_team_inbox_assignment.py`<br>`tests/test_team_inbox_commands.py`<br>`tests/test_team_inbox_fifo_queue.py`<br>`tests/test_team_inbox_queue_notifications.py`<br>`tests/integration/test_team_inbox_queue_concurrency.py` |
| `communications.team_inbox_routing` | current customer-visible queue position projection | `resolver` | conversation routing facts ← `communications.team_inbox_threads`<br>operational escalation policy ← `operations.sla_escalation`<br>operator authorization ← `auth.permission_gate`<br>validated AI intake destination metadata ← `ai.intake`<br>active AI conversation ownership ← `ai.intake`<br>successful staff session issuance ← `app_sessions.auth`<br>authenticated visible Inbox activity ← `app_sessions.auth`<br>active staff principal ← `auth.staff_provisioning`<br>agent capacity configuration ← `control.settings_spec`<br>WhatsApp service-window state ← `communications.team_inbox_reply_window` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_admin_inbox_slice4_workflows.py`<br>`tests/test_auth_flow.py`<br>`tests/test_team_inbox_assignment.py`<br>`tests/test_team_inbox_commands.py`<br>`tests/test_team_inbox_fifo_queue.py`<br>`tests/test_team_inbox_queue_notifications.py`<br>`tests/integration/test_team_inbox_queue_concurrency.py` |
| `communications.team_inbox_routing` | global per-agent active assignment capacity enforcement | `policy` | conversation routing facts ← `communications.team_inbox_threads`<br>operational escalation policy ← `operations.sla_escalation`<br>operator authorization ← `auth.permission_gate`<br>validated AI intake destination metadata ← `ai.intake`<br>active AI conversation ownership ← `ai.intake`<br>successful staff session issuance ← `app_sessions.auth`<br>authenticated visible Inbox activity ← `app_sessions.auth`<br>active staff principal ← `auth.staff_provisioning`<br>agent capacity configuration ← `control.settings_spec`<br>WhatsApp service-window state ← `communications.team_inbox_reply_window` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_admin_inbox_slice4_workflows.py`<br>`tests/test_auth_flow.py`<br>`tests/test_team_inbox_assignment.py`<br>`tests/test_team_inbox_commands.py`<br>`tests/test_team_inbox_fifo_queue.py`<br>`tests/test_team_inbox_queue_notifications.py`<br>`tests/integration/test_team_inbox_queue_concurrency.py` |
...
| `communications.team_inbox_routing` | current agent presence state and freshness | `authoritative_record` | conversation routing facts ← `communications.team_inbox_threads`<br>operational escalation policy ← `operations.sla_escalation`<br>operator authorization ← `auth.permission_gate`<br>validated AI intake destination metadata ← `ai.intake`<br>active AI conversation ownership ← `ai.intake`<br>successful staff session issuance ← `app_sessions.auth`<br>authenticated visible Inbox activity ← `app_sessions.auth`<br>active staff principal ← `auth.staff_provisioning`<br>agent capacity configuration ← `control.settings_spec`<br>WhatsApp service-window state ← `communications.team_inbox_reply_window` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_admin_inbox_slice4_workflows.py`<br>`tests/test_auth_flow.py`<br>`tests/test_team_inbox_assignment.py`<br>`tests/test_team_inbox_commands.py`<br>`tests/test_team_inbox_fifo_queue.py`<br>`tests/test_team_inbox_queue_notifications.py`<br>`tests/integration/test_team_inbox_queue_concurrency.py` |
| `communications.team_inbox_routing` | agent presence transitions | `command_writer` | conversation routing facts ← `communications.team_inbox_threads`<br>operational escalation policy ← `operations.sla_escalation`<br>operator authorization ← `auth.permission_gate`<br>validated AI intake destination metadata ← `ai.intake`<br>active AI conversation ownership ← `ai.intake`<br>successful staff session issuance ← `app_sessions.auth`<br>authenticated visible Inbox activity ← `app_sessions.auth`<br>active staff principal ← `auth.staff_provisioning`<br>agent capacity configuration ← `control.settings_spec`<br>WhatsApp service-window state ← `communications.team_inbox_reply_window` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_admin_inbox_slice4_workflows.py`<br>`tests/test_auth_flow.py`<br>`tests/test_team_inbox_assignment.py`<br>`tests/test_team_inbox_commands.py`<br>`tests/test_team_inbox_fifo_queue.py`<br>`tests/test_team_inbox_queue_notifications.py`<br>`tests/integration/test_team_inbox_queue_concurrency.py` |
| `communications.team_inbox_routing` | immutable agent presence transition evidence | `authoritative_record` | conversation routing facts ← `communications.team_inbox_threads`<br>operational escalation policy ← `operations.sla_escalation`<br>operator authorization ← `auth.permission_gate`<br>validated AI intake destination metadata ← `ai.intake`<br>active AI conversation ownership ← `ai.intake`<br>successful staff session issuance ← `app_sessions.auth`<br>authenticated visible Inbox activity ← `app_sessions.auth`<br>active staff principal ← `auth.staff_provisioning`<br>agent capacity configuration ← `control.settings_spec`<br>WhatsApp service-window state ← `communications.team_inbox_reply_window` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_admin_inbox_slice4_workflows.py`<br>`tests/test_auth_flow.py`<br>`tests/test_team_inbox_assignment.py`<br>`tests/test_team_inbox_commands.py`<br>`tests/test_team_inbox_fifo_queue.py`<br>`tests/test_team_inbox_queue_notifications.py`<br>`tests/integration/test_team_inbox_queue_concurrency.py` |
| `communications.team_inbox_routing` | expired WhatsApp assignment and FIFO release | `command_writer` | conversation routing facts ← `communications.team_inbox_threads`<br>operational escalation policy ← `operations.sla_escalation`<br>operator authorization ← `auth.permission_gate`<br>validated AI intake destination metadata ← `ai.intake`<br>active AI conversation ownership ← `ai.intake`<br>successful staff session issuance ← `app_sessions.auth`<br>authenticated visible Inbox activity ← `app_sessions.auth`<br>active staff principal ← `auth.staff_provisioning`<br>agent capacity configuration ← `control.settings_spec`<br>WhatsApp service-window state ← `communications.team_inbox_reply_window` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_admin_inbox_slice4_workflows.py`<br>`tests/test_auth_flow.py`<br>`tests/test_team_inbox_assignment.py`<br>`tests/test_team_inbox_commands.py`<br>`tests/test_team_inbox_fifo_queue.py`<br>`tests/test_team_inbox_queue_notifications.py`<br>`tests/integration/test_team_inbox_queue_concurrency.py` |
...
| `communications.team_inbox_queue_notifications` | queue notification lifecycle deduplication and suppression | `policy` | FIFO queue entry state ← `communications.team_inbox_routing`<br>customer outbound delivery result ← `communications.team_inbox_outbound_intents` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_queue_notifications.py` |
| `communications.team_inbox_queue_notifications` | queue notification provider-dispatch validity decision | `policy` | FIFO queue entry state ← `communications.team_inbox_routing`<br>customer outbound delivery result ← `communications.team_inbox_outbound_intents` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_queue_notifications.py` |
| `communications.team_inbox_automation` | Team Inbox automation trigger matching | `policy` | active AI conversation ownership ← `ai.intake`<br>conversation trigger facts ← `communications.team_inbox_threads`<br>routing and collaboration commands ← `communications.team_inbox_routing` | `coordinator_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_automation.py` |
| `communications.team_inbox_automation` | ordered Inbox automation action execution | `application_coordinator` | active AI conversation ownership ← `ai.intake`<br>conversation trigger facts ← `communications.team_inbox_threads`<br>routing and collaboration commands ← `communications.team_inbox_routing` | `coordinator_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_automation.py` |
...
| `communications.team_inbox_status` | conversation status transitions and immutable evidence | `command_writer` | current conversation status ← `communications.team_inbox_threads`<br>typed status transition command ← `auth.permission_gate`<br>Customer resolution readiness ← `communications.team_inbox_customer_completion`<br>Meta channel resolution state ← `communications.team_inbox_reply_window` | `participant` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/designs/INBOX_CUSTOMER_COMPLETION_GATE.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_lifecycle_audit.py`<br>`tests/test_inbox_customer_completion.py`<br>`tests/architecture/test_inbox_customer_completion_boundary.py`<br>`tests/architecture/test_team_inbox_lifecycle_audit_boundary.py` |
| `communications.team_inbox_status` | audited expired conversation resolution | `command_writer` | current conversation status ← `communications.team_inbox_threads`<br>typed status transition command ← `auth.permission_gate`<br>Customer resolution readiness ← `communications.team_inbox_customer_completion`<br>Meta channel resolution state ← `communications.team_inbox_reply_window` | `participant` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/designs/INBOX_CUSTOMER_COMPLETION_GATE.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_lifecycle_audit.py`<br>`tests/test_inbox_customer_completion.py`<br>`tests/architecture/test_inbox_customer_completion_boundary.py`<br>`tests/architecture/test_team_inbox_lifecycle_audit_boundary.py` |
| `communications.team_inbox_completion_override` | single-use legacy customer-completion resolution override | `command_writer` | pre-cutover eligibility marker ← `communications.team_inbox_threads`<br>live Customer resolution readiness ← `communications.team_inbox_customer_completion`<br>typed override grant/consumption command ← `auth.permission_gate` | `owner_managed` | `complete` | customer experience platform | `docs/designs/INBOX_CUSTOMER_COMPLETION_GATE.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_completion_override.py`<br>`tests/architecture/test_inbox_completion_override_boundary.py` |
...
| `communications.team_inbox_outbound_intents` | transactional outbound communication intent | `command_writer` | active AI conversation ownership ← `ai.intake`<br>conversation reply target ← `communications.team_inbox_threads`<br>communication intent lifecycle ← `communications.intents`<br>effective channel policy ← `communications.channel_policy`<br>accepted outbound provider message observation ← `external:communications_provider` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_sot_completion.py`<br>`tests/architecture/test_team_inbox_boundaries.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py` |
| `communications.team_inbox_outbound_intents` | outbound Inbox message attempt projection | `projection_writer` | active AI conversation ownership ← `ai.intake`<br>conversation reply target ← `communications.team_inbox_threads`<br>communication intent lifecycle ← `communications.intents`<br>effective channel policy ← `communications.channel_policy`<br>accepted outbound provider message observation ← `external:communications_provider` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_sot_completion.py`<br>`tests/architecture/test_team_inbox_boundaries.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py` |
...
| `communications.team_inbox_commands` | operator conversation and collaboration commands | `application_coordinator` | active AI conversation ownership ← `ai.intake`<br>authenticated operator command ← `auth.permission_gate`<br>current conversation state ← `communications.team_inbox_threads`<br>contact association decision ← `communications.team_inbox_contact_resolution`<br>conversation Lead relationship decision ← `communications.conversation_lead_relationships`<br>routing transition decision ← `communications.team_inbox_routing`<br>outbound intent outcome ← `communications.team_inbox_outbound_intents`<br>notification delivery state ← `communications.notification_service`<br>operator read state ← `communications.team_inbox_operator_state`<br>staff notification staging outcome ← `communications.staff_notifications`<br>optional staff Talk admission outcome ← `communications.nextcloud_talk_staff` | `coordinator_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_sot_completion.py`<br>`tests/architecture/test_team_inbox_boundaries.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py` |
| `communications.team_inbox_commands` | queued AI outbound ownership revalidation and suppression | `application_coordinator` | active AI conversation ownership ← `ai.intake`<br>authenticated operator command ← `auth.permission_gate`<br>current conversation state ← `communications.team_inbox_threads`<br>contact association decision ← `communications.team_inbox_contact_resolution`<br>conversation Lead relationship decision ← `communications.conversation_lead_relationships`<br>routing transition decision ← `communications.team_inbox_routing`<br>outbound intent outcome ← `communications.team_inbox_outbound_intents`<br>notification delivery state ← `communications.notification_service`<br>operator read state ← `communications.team_inbox_operator_state`<br>staff notification staging outcome ← `communications.staff_notifications`<br>optional staff Talk admission outcome ← `communications.nextcloud_talk_staff` | `coordinator_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_sot_completion.py`<br>`tests/architecture/test_team_inbox_boundaries.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py` |
| `communications.team_inbox_widget` | visitor chat session, message, and read-state commands | `command_writer` | authenticated visitor principal ← `customer.identity_scope`<br>anonymous fiber visitor command ← `communications.team_inbox_widget`<br>fiber visitor contact resolution ← `communications.team_inbox_contact_resolution`<br>fiber visitor prospect capture ← `sales.capture`<br>widget conversation identity ← `communications.team_inbox_threads`<br>enabled matching AI intake configuration ← `ai.intake` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/adr/0006-temporary-crm-chat-authority.md`<br>`docs/CHAT_LIVE_SETUP.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_chat_session.py`<br>`tests/test_team_inbox_widget_native.py`<br>`tests/architecture/test_single_chat_authority.py`<br>`tests/architecture/test_team_inbox_boundaries.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py` |
...
| `communications.team_inbox_projection` | Inbox list detail metrics response cohort unread and action projection | `resolver` | active AI conversation ownership ← `ai.intake`<br>conversation records ← `communications.team_inbox_threads`<br>contact projection ← `communications.team_inbox_contact_resolution`<br>routing state ← `communications.team_inbox_routing`<br>delivery projection ← `communications.team_inbox_delivery_receipts`<br>unread projection ← `communications.team_inbox_operator_state`<br>ticket handoff provenance ← `communications.conversation_ticket_handoff`<br>active service-team selector projection ← `operations.service_team_lifecycle`<br>canonical staff display identity ← `auth.staff_provisioning`<br>Inbox media content facts ← `communications.team_inbox_threads`<br>Inbox structured location facts ← `communications.team_inbox_threads` | `read_only` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_sot_completion.py`<br>`tests/test_team_inbox_read.py`<br>`tests/test_team_inbox_needs_attention.py`<br>`tests/test_team_inbox_filters.py`<br>`tests/test_team_inbox_attachments.py`<br>`tests/test_admin_inbox_workspace.py`<br>`tests/test_admin_inbox_routes_http.py`<br>`tests/test_admin_inbox_workspace_integrity.py`<br>`tests/architecture/test_team_inbox_boundaries.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py`<br>`tests/architecture/test_team_inbox_ai_ownership_boundary.py` |
| `communications.team_inbox_projection` | Inbox outbound message sender identity projection | `resolver` | active AI conversation ownership ← `ai.intake`<br>conversation records ← `communications.team_inbox_threads`<br>contact projection ← `communications.team_inbox_contact_resolution`<br>routing state ← `communications.team_inbox_routing`<br>delivery projection ← `communications.team_inbox_delivery_receipts`<br>unread projection ← `communications.team_inbox_operator_state`<br>ticket handoff provenance ← `communications.conversation_ticket_handoff`<br>active service-team selector projection ← `operations.service_team_lifecycle`<br>canonical staff display identity ← `auth.staff_provisioning`<br>Inbox media content facts ← `communications.team_inbox_threads`<br>Inbox structured location facts ← `communications.team_inbox_threads` | `read_only` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_sot_completion.py`<br>`tests/test_team_inbox_read.py`<br>`tests/test_team_inbox_needs_attention.py`<br>`tests/test_team_inbox_filters.py`<br>`tests/test_team_inbox_attachments.py`<br>`tests/test_admin_inbox_workspace.py`<br>`tests/test_admin_inbox_routes_http.py`<br>`tests/test_admin_inbox_workspace_integrity.py`<br>`tests/architecture/test_team_inbox_boundaries.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py`<br>`tests/architecture/test_team_inbox_ai_ownership_boundary.py` |
| `communications.team_inbox_projection` | Inbox email recipient envelope projection | `resolver` | active AI conversation ownership ← `ai.intake`<br>conversation records ← `communications.team_inbox_threads`<br>contact projection ← `communications.team_inbox_contact_resolution`<br>routing state ← `communications.team_inbox_routing`<br>delivery projection ← `communications.team_inbox_delivery_receipts`<br>unread projection ← `communications.team_inbox_operator_state`<br>ticket handoff provenance ← `communications.conversation_ticket_handoff`<br>active service-team selector projection ← `operations.service_team_lifecycle`<br>canonical staff display identity ← `auth.staff_provisioning`<br>Inbox media content facts ← `communications.team_inbox_threads`<br>Inbox structured location facts ← `communications.team_inbox_threads` | `read_only` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_sot_completion.py`<br>`tests/test_team_inbox_read.py`<br>`tests/test_team_inbox_needs_attention.py`<br>`tests/test_team_inbox_filters.py`<br>`tests/test_team_inbox_attachments.py`<br>`tests/test_admin_inbox_workspace.py`<br>`tests/test_admin_inbox_routes_http.py`<br>`tests/test_admin_inbox_workspace_integrity.py`<br>`tests/architecture/test_team_inbox_boundaries.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py`<br>`tests/architecture/test_team_inbox_ai_ownership_boundary.py` |
| `communications.team_inbox_projection` | Inbox media browser presentation projection | `resolver` | active AI conversation ownership ← `ai.intake`<br>conversation records ← `communications.team_inbox_threads`<br>contact projection ← `communications.team_inbox_contact_resolution`<br>routing state ← `communications.team_inbox_routing`<br>delivery projection ← `communications.team_inbox_delivery_receipts`<br>unread projection ← `communications.team_inbox_operator_state`<br>ticket handoff provenance ← `communications.conversation_ticket_handoff`<br>active service-team selector projection ← `operations.service_team_lifecycle`<br>canonical staff display identity ← `auth.staff_provisioning`<br>Inbox media content facts ← `communications.team_inbox_threads`<br>Inbox structured location facts ← `communications.team_inbox_threads` | `read_only` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_sot_completion.py`<br>`tests/test_team_inbox_read.py`<br>`tests/test_team_inbox_needs_attention.py`<br>`tests/test_team_inbox_filters.py`<br>`tests/test_team_inbox_attachments.py`<br>`tests/test_admin_inbox_workspace.py`<br>`tests/test_admin_inbox_routes_http.py`<br>`tests/test_admin_inbox_workspace_integrity.py`<br>`tests/architecture/test_team_inbox_boundaries.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py`<br>`tests/architecture/test_team_inbox_ai_ownership_boundary.py` |
| `communications.team_inbox_projection` | Inbox structured location browser presentation projection | `resolver` | active AI conversation ownership ← `ai.intake`<br>conversation records ← `communications.team_inbox_threads`<br>contact projection ← `communications.team_inbox_contact_resolution`<br>routing state ← `communications.team_inbox_routing`<br>delivery projection ← `communications.team_inbox_delivery_receipts`<br>unread projection ← `communications.team_inbox_operator_state`<br>ticket handoff provenance ← `communications.conversation_ticket_handoff`<br>active service-team selector projection ← `operations.service_team_lifecycle`<br>canonical staff display identity ← `auth.staff_provisioning`<br>Inbox media content facts ← `communications.team_inbox_threads`<br>Inbox structured location facts ← `communications.team_inbox_threads` | `read_only` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_sot_completion.py`<br>`tests/test_team_inbox_read.py`<br>`tests/test_team_inbox_needs_attention.py`<br>`tests/test_team_inbox_filters.py`<br>`tests/test_team_inbox_attachments.py`<br>`tests/test_admin_inbox_workspace.py`<br>`tests/test_admin_inbox_routes_http.py`<br>`tests/test_admin_inbox_workspace_integrity.py`<br>`tests/architecture/test_team_inbox_boundaries.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py`<br>`tests/architecture/test_team_inbox_ai_ownership_boundary.py` |
| `communications.team_inbox_metrics` | bounded Inbox team performance report projection | `resolver` | conversation and message chronology ← `communications.team_inbox_threads`<br>assignment routing and presence state ← `communications.team_inbox_routing`<br>service-team identity and lifecycle ← `operations.service_team_lifecycle`<br>service-team capability projection ← `operations.service_team_composition`<br>staff identity ← `auth.staff_provisioning` | `read_only` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_metrics.py`<br>`tests/test_crm_reporting.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py` |
| `communications.team_inbox_metrics` | bounded Inbox agent performance report projection | `resolver` | conversation and message chronology ← `communications.team_inbox_threads`<br>assignment routing and presence state ← `communications.team_inbox_routing`<br>service-team identity and lifecycle ← `operations.service_team_lifecycle`<br>service-team capability projection ← `operations.service_team_composition`<br>staff identity ← `auth.staff_provisioning` | `read_only` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_metrics.py`<br>`tests/test_crm_reporting.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py` |
| `communications.team_inbox_metrics` | current Inbox escalation candidate projection | `resolver` | conversation and message chronology ← `communications.team_inbox_threads`<br>assignment routing and presence state ← `communications.team_inbox_routing`<br>service-team identity and lifecycle ← `operations.service_team_lifecycle`<br>service-team capability projection ← `operations.service_team_composition`<br>staff identity ← `auth.staff_provisioning` | `read_only` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_metrics.py`<br>`tests/test_crm_reporting.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py` |
...
| `communications.team_inbox_maintenance` | scheduled Inbox projection maintenance and repair | `reconciler` | current Inbox projection ← `communications.team_inbox_projection`<br>canonical conversation identity ← `communications.team_inbox_threads`<br>outbound intent state ← `communications.team_inbox_outbound_intents`<br>AI intake recovery state ← `ai.intake`<br>normal human routing and FIFO state ← `communications.team_inbox_routing`<br>WhatsApp expiry worklist ← `communications.team_inbox_reply_window`<br>verified WhatsApp webhook repair evidence ← `integration.inbox`<br>Meta contact profile observation ← `integration.runtime` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_sot_completion.py`<br>`tests/architecture/test_team_inbox_boundaries.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py` |
| `communications.team_inbox_maintenance` | WhatsApp expiry routing-state convergence | `reconciler` | current Inbox projection ← `communications.team_inbox_projection`<br>canonical conversation identity ← `communications.team_inbox_threads`<br>outbound intent state ← `communications.team_inbox_outbound_intents`<br>AI intake recovery state ← `ai.intake`<br>normal human routing and FIFO state ← `communications.team_inbox_routing`<br>WhatsApp expiry worklist ← `communications.team_inbox_reply_window`<br>verified WhatsApp webhook repair evidence ← `integration.inbox`<br>Meta contact profile observation ← `integration.runtime` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_sot_completion.py`<br>`tests/architecture/test_team_inbox_boundaries.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py` |
...
| `communications.team_inbox_health` | verified SMTP probe delivery projection | `projection_writer` | exact synthetic provider-stable probe ID ← `communications.team_inbox_threads` | `owner_managed` | `complete` | customer experience platform | `docs/designs/TEAM_INBOX_SOURCE_OF_TRUTH.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_team_inbox_sot_completion.py`<br>`tests/architecture/test_team_inbox_boundaries.py`<br>`tests/architecture/test_team_inbox_sot_contracts.py` |
...
| `communications.inbox_sla` | Inbox SLA policy selection and clock state | `authoritative_record` | Inbox conversation facts ← `communications.team_inbox_threads`<br>Inbox SLA commands ← `communications.inbox_sla` | `owner_managed` | `complete` | customer experience platform | `docs/designs/INBOX_SLA.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_inbox_sla.py`<br>`tests/architecture/test_inbox_sla_command_boundary.py` |
| `communications.inbox_sla` | Inbox SLA transition evidence | `authoritative_record` | Inbox conversation facts ← `communications.team_inbox_threads`<br>Inbox SLA commands ← `communications.inbox_sla` | `owner_managed` | `complete` | customer experience platform | `docs/designs/INBOX_SLA.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_inbox_sla.py`<br>`tests/architecture/test_inbox_sla_command_boundary.py` |
...
| `runtime.web_worker_readiness` | web-worker route-table readiness | `policy` | mounted supported route table ← `runtime.web_worker_readiness`<br>essential startup preflight ← `runtime.web_worker_readiness` | `not_applicable` | `native` | platform runtime | `docs/runbooks/WEB_WORKER_READINESS.md`<br>`tests/architecture/test_startup_router_readiness.py` |
...
| `observability.structured_operational_logs` | structured operational task-summary observations | `resolver` | typed operational task outcome ← `observability.structured_operational_logs` | `not_applicable` | `native` | platform operations | `docs/designs/OPERATIONAL_EVIDENCE_AND_RETRY.md`<br>`tests/test_structured_operational_logging.py` |
...
| `observability.application_failures` | unhandled HTTP exception observations | `resolver` | unhandled HTTP exception ← `observability.application_failures` | `not_applicable` | `native` | platform operations | `docs/designs/OPERATIONAL_EVIDENCE_AND_RETRY.md`<br>`tests/test_application_exception_observability.py` |
| `observability.application_failures` | payment verification outcome observations | `resolver` | payment verification outcome ← `observability.application_failures` | `not_applicable` | `native` | platform operations | `docs/designs/OPERATIONAL_EVIDENCE_AND_RETRY.md`<br>`tests/test_application_exception_observability.py` |
...
| `operations.field_location_ingest` | field-location ping ingest | `command_writer` | authenticated field technician principal ← `auth.staff_provisioning`<br>submitted field-location ping ← `operations.field_location_ingest` | `owner_managed` | `native` | field operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_field_location_tracking.py` |
| `operations.field_location_ingest` | field-location sharing preference | `command_writer` | authenticated field technician principal ← `auth.staff_provisioning`<br>field-location sharing preference command ← `operations.field_location_ingest` | `owner_managed` | `native` | field operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_field_location_tracking.py` |
...
| `support.ticket_lifecycle` | ticket lifecycle mutations | `command_writer` | typed ticket command ← `support.ticket_lifecycle`<br>canonical ticket state ← `support.ticket_lifecycle`<br>ticket configuration ← `support.ticket_configuration`<br>portal team-routing resolution ← `support.ticket_configuration`<br>customer identity evidence ← `customer.identity_scope`<br>assignment policy proposal ← `support.ticket_assignment_evaluation`<br>automation policy proposal ← `support.ticket_automation_evaluation` | `owner_managed` | `complete` | support operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/SUPPORT_TICKET_LIFECYCLE_SOT.md`<br>`docs/designs/SUPPORT_UX_POLISH_AUDIT.md`<br>`docs/REALTIME_PLATFORM.md`<br>`mobile/README.md`<br>`tests/test_support_services.py`<br>`tests/test_ticket_status_transition.py`<br>`tests/test_support_automation.py`<br>`tests/test_ticket_assignment_engine.py`<br>`tests/test_ticket_assignment_authorization.py`<br>`tests/test_ticket_assignment_role_grant_migration.py`<br>`tests/test_support_ticket_comment_realtime.py`<br>`tests/architecture/test_support_ticket_sot_boundary.py` |
...
| `support.ticket_lifecycle` | ticket staff/team tag notification consequence | `event_policy` | typed ticket command ← `support.ticket_lifecycle`<br>canonical ticket state ← `support.ticket_lifecycle`<br>staff notification delivery queue ← `communications.notification_service` | `owner_managed` | `complete` | support operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/SUPPORT_TICKET_LIFECYCLE_SOT.md`<br>`docs/designs/SUPPORT_UX_POLISH_AUDIT.md`<br>`docs/REALTIME_PLATFORM.md`<br>`mobile/README.md`<br>`tests/test_support_services.py`<br>`tests/test_ticket_status_transition.py`<br>`tests/test_support_automation.py`<br>`tests/test_ticket_assignment_engine.py`<br>`tests/test_ticket_assignment_authorization.py`<br>`tests/test_ticket_assignment_role_grant_migration.py`<br>`tests/test_support_ticket_comment_realtime.py`<br>`tests/architecture/test_support_ticket_sot_boundary.py` |
| `support.ticket_lifecycle` | customer-visible ticket comment realtime invalidation policy | `event_policy` | typed ticket command ← `support.ticket_lifecycle`<br>canonical ticket state ← `support.ticket_lifecycle`<br>best-effort realtime projection transport ← `runtime.realtime_projection` | `owner_managed` | `complete` | support operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/SUPPORT_TICKET_LIFECYCLE_SOT.md`<br>`docs/designs/SUPPORT_UX_POLISH_AUDIT.md`<br>`docs/REALTIME_PLATFORM.md`<br>`mobile/README.md`<br>`tests/test_support_services.py`<br>`tests/test_ticket_status_transition.py`<br>`tests/test_support_automation.py`<br>`tests/test_ticket_assignment_engine.py`<br>`tests/test_ticket_assignment_authorization.py`<br>`tests/test_ticket_assignment_role_grant_migration.py`<br>`tests/test_support_ticket_comment_realtime.py`<br>`tests/architecture/test_support_ticket_sot_boundary.py` |
...
| `support.ticket_region_projection` | canonical support-ticket region projection | `resolver` | current ticket configuration ← `support.ticket_configuration` | `read_only` | `complete` | support operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/SUPPORT_TICKET_LIFECYCLE_SOT.md`<br>`tests/test_support_ticket_settings.py`<br>`tests/test_sot_relationships.py` |
...
| `operations.field_notes` | native field work-order note creation | `command_writer` | authenticated technician identity ← `auth.permission_gate`<br>assigned work-order state ← `operations.work_orders`<br>field-note client request identity ← `operations.field_notes` | `owner_managed` | `cutover_ready` | field operations | `docs/designs/FIELD_WORK_ORDER_NOTES.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_field_notes.py`<br>`tests/test_field_note_staff_projection.py`<br>`tests/architecture/test_field_note_delivery_contract.py`<br>`tests/integration/test_field_note_delivery_migration.py`<br>`field_mobile/test/execution_test.dart`<br>`field_mobile/test/jobs_screens_test.dart` |
| `operations.field_notes` | authorized staff field-note related-context projection | `resolver` | canonical field work-order note state ← `operations.field_notes`<br>native work-order relationship bindings ← `operations.work_order_commands`<br>staff field-note authorization evidence ← `auth.permission_gate` | `owner_managed` | `cutover_ready` | field operations | `docs/designs/FIELD_WORK_ORDER_NOTES.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_field_notes.py`<br>`tests/test_field_note_staff_projection.py`<br>`tests/architecture/test_field_note_delivery_contract.py`<br>`tests/integration/test_field_note_delivery_migration.py`<br>`field_mobile/test/execution_test.dart`<br>`field_mobile/test/jobs_screens_test.dart` |
...
| `operations.expense_requests` | field expense request submission | `command_writer` | canonical service work-order state ← `operations.work_orders`<br>authenticated requester and work-order access evidence ← `auth.permission_gate`<br>ERP-owned expense category rules ← `operations.expense_categories`<br>ERP-owned eligible approver observation ← `external:dotmac_erp`<br>ERP-verified payment destination token ← `external:dotmac_erp`<br>validated receipt content ← `operations.expense_requests` | `owner_managed` | `cutover_ready` | field operations and finance | `docs/designs/WORK_ORDER_EXPENSE_ENTRY.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/runbooks/EXPENSE_CLAIM_ERP_CUTOVER.md`<br>`tests/test_field_expense_requests.py`<br>`tests/test_field_manager_api.py`<br>`tests/test_work_order_expense_web.py`<br>`tests/test_dotmac_erp_expense_sync.py`<br>`tests/playwright/e2e/test_work_order_expense_disclosure.py`<br>`tests/architecture/test_field_expense_request_writer_boundary.py`<br>`tests/architecture/test_field_request_history_identity.py`<br>`tests/integration/test_field_request_requester_history_migration.py` |
| `operations.expense_requests` | field expense request cancellation | `command_writer` | canonical field expense request state ← `operations.expense_requests`<br>authenticated requester and work-order access evidence ← `auth.permission_gate` | `owner_managed` | `cutover_ready` | field operations and finance | `docs/designs/WORK_ORDER_EXPENSE_ENTRY.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/runbooks/EXPENSE_CLAIM_ERP_CUTOVER.md`<br>`tests/test_field_expense_requests.py`<br>`tests/test_field_manager_api.py`<br>`tests/test_work_order_expense_web.py`<br>`tests/test_dotmac_erp_expense_sync.py`<br>`tests/playwright/e2e/test_work_order_expense_disclosure.py`<br>`tests/architecture/test_field_expense_request_writer_boundary.py`<br>`tests/architecture/test_field_request_history_identity.py`<br>`tests/integration/test_field_request_requester_history_migration.py` |
| `operations.expense_requests` | field expense ERP form context | `resolver` | authenticated requester and work-order access evidence ← `auth.permission_gate`<br>ERP-owned eligible approver observation ← `external:dotmac_erp`<br>ERP-verified payment destination token ← `external:dotmac_erp` | `owner_managed` | `cutover_ready` | field operations and finance | `docs/designs/WORK_ORDER_EXPENSE_ENTRY.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/runbooks/EXPENSE_CLAIM_ERP_CUTOVER.md`<br>`tests/test_field_expense_requests.py`<br>`tests/test_field_manager_api.py`<br>`tests/test_work_order_expense_web.py`<br>`tests/test_dotmac_erp_expense_sync.py`<br>`tests/playwright/e2e/test_work_order_expense_disclosure.py`<br>`tests/architecture/test_field_expense_request_writer_boundary.py`<br>`tests/architecture/test_field_request_history_identity.py`<br>`tests/integration/test_field_request_requester_history_migration.py` |
| `operations.expense_requests` | expense receipt staging for submitted claims | `command_writer` | canonical service work-order state ← `operations.work_orders`<br>authenticated requester and work-order access evidence ← `auth.permission_gate`<br>validated receipt content ← `operations.expense_requests` | `owner_managed` | `cutover_ready` | field operations and finance | `docs/designs/WORK_ORDER_EXPENSE_ENTRY.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/runbooks/EXPENSE_CLAIM_ERP_CUTOVER.md`<br>`tests/test_field_expense_requests.py`<br>`tests/test_field_manager_api.py`<br>`tests/test_work_order_expense_web.py`<br>`tests/test_dotmac_erp_expense_sync.py`<br>`tests/playwright/e2e/test_work_order_expense_disclosure.py`<br>`tests/architecture/test_field_expense_request_writer_boundary.py`<br>`tests/architecture/test_field_request_history_identity.py`<br>`tests/integration/test_field_request_requester_history_migration.py` |
| `operations.expense_requests` | field expense lifecycle ERP delivery staging | `command_writer` | canonical submitted expense request ← `operations.expense_requests`<br>selected expense approver ← `operations.expense_requests`<br>ERP-owned expense category rules ← `operations.expense_categories`<br>validated receipt content ← `operations.expense_requests`<br>expense ERP delivery cutover control ← `integration.backoffice_adapter` | `owner_managed` | `cutover_ready` | field operations and finance | `docs/designs/WORK_ORDER_EXPENSE_ENTRY.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/runbooks/EXPENSE_CLAIM_ERP_CUTOVER.md`<br>`tests/test_field_expense_requests.py`<br>`tests/test_field_manager_api.py`<br>`tests/test_work_order_expense_web.py`<br>`tests/test_dotmac_erp_expense_sync.py`<br>`tests/playwright/e2e/test_work_order_expense_disclosure.py`<br>`tests/architecture/test_field_expense_request_writer_boundary.py`<br>`tests/architecture/test_field_request_history_identity.py`<br>`tests/integration/test_field_request_requester_history_migration.py` |
| `operations.expense_requests` | dead expense delivery recovery | `command_writer` | canonical approved expense request ← `operations.expense_requests`<br>ERP-owned expense category rules ← `operations.expense_categories`<br>validated receipt content ← `operations.expense_requests`<br>expense ERP delivery cutover control ← `integration.backoffice_adapter` | `owner_managed` | `cutover_ready` | field operations and finance | `docs/designs/WORK_ORDER_EXPENSE_ENTRY.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/runbooks/EXPENSE_CLAIM_ERP_CUTOVER.md`<br>`tests/test_field_expense_requests.py`<br>`tests/test_field_manager_api.py`<br>`tests/test_work_order_expense_web.py`<br>`tests/test_dotmac_erp_expense_sync.py`<br>`tests/playwright/e2e/test_work_order_expense_disclosure.py`<br>`tests/architecture/test_field_expense_request_writer_boundary.py`<br>`tests/architecture/test_field_request_history_identity.py`<br>`tests/integration/test_field_request_requester_history_migration.py` |
| `operations.expense_requests` | dead expense payment delivery recovery | `command_writer` | canonical approved expense request ← `operations.expense_requests`<br>permission-denied ERP payment delivery evidence ← `integration.backoffice_adapter`<br>ERP expense claim and payment status observation ← `external:dotmac_erp`<br>expense ERP delivery cutover control ← `integration.backoffice_adapter` | `owner_managed` | `cutover_ready` | field operations and finance | `docs/designs/WORK_ORDER_EXPENSE_ENTRY.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/runbooks/EXPENSE_CLAIM_ERP_CUTOVER.md`<br>`tests/test_field_expense_requests.py`<br>`tests/test_field_manager_api.py`<br>`tests/test_work_order_expense_web.py`<br>`tests/test_dotmac_erp_expense_sync.py`<br>`tests/playwright/e2e/test_work_order_expense_disclosure.py`<br>`tests/architecture/test_field_expense_request_writer_boundary.py`<br>`tests/architecture/test_field_request_history_identity.py`<br>`tests/integration/test_field_request_requester_history_migration.py` |
| `operations.expense_requests` | field expense payment initiation and ERP delivery staging | `command_writer` | canonical approved expense request ← `operations.expense_requests`<br>expense ERP delivery cutover control ← `integration.backoffice_adapter` | `owner_managed` | `cutover_ready` | field operations and finance | `docs/designs/WORK_ORDER_EXPENSE_ENTRY.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/runbooks/EXPENSE_CLAIM_ERP_CUTOVER.md`<br>`tests/test_field_expense_requests.py`<br>`tests/test_field_manager_api.py`<br>`tests/test_work_order_expense_web.py`<br>`tests/test_dotmac_erp_expense_sync.py`<br>`tests/playwright/e2e/test_work_order_expense_disclosure.py`<br>`tests/architecture/test_field_expense_request_writer_boundary.py`<br>`tests/architecture/test_field_request_history_identity.py`<br>`tests/integration/test_field_request_requester_history_migration.py` |
| `operations.expense_requests` | ERP expense-claim payment outcome observation | `reconciler` | canonical field expense request state ← `operations.expense_requests`<br>ERP expense claim and payment status observation ← `external:dotmac_erp`<br>expense ERP delivery cutover control ← `integration.backoffice_adapter` | `owner_managed` | `cutover_ready` | field operations and finance | `docs/designs/WORK_ORDER_EXPENSE_ENTRY.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/runbooks/EXPENSE_CLAIM_ERP_CUTOVER.md`<br>`tests/test_field_expense_requests.py`<br>`tests/test_field_manager_api.py`<br>`tests/test_work_order_expense_web.py`<br>`tests/test_dotmac_erp_expense_sync.py`<br>`tests/playwright/e2e/test_work_order_expense_disclosure.py`<br>`tests/architecture/test_field_expense_request_writer_boundary.py`<br>`tests/architecture/test_field_request_history_identity.py`<br>`tests/integration/test_field_request_requester_history_migration.py` |
| `operations.expense_requests` | field expense vendor picker | `resolver` | legacy active vendor identity observation ← `external:legacy_vendor_registry` | `owner_managed` | `cutover_ready` | field operations and finance | `docs/designs/WORK_ORDER_EXPENSE_ENTRY.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/runbooks/EXPENSE_CLAIM_ERP_CUTOVER.md`<br>`tests/test_field_expense_requests.py`<br>`tests/test_field_manager_api.py`<br>`tests/test_work_order_expense_web.py`<br>`tests/test_dotmac_erp_expense_sync.py`<br>`tests/playwright/e2e/test_work_order_expense_disclosure.py`<br>`tests/architecture/test_field_expense_request_writer_boundary.py`<br>`tests/architecture/test_field_request_history_identity.py`<br>`tests/integration/test_field_request_requester_history_migration.py` |
| `operations.expense_requests` | requester-owned field expense history | `resolver` | canonical field expense request state ← `operations.expense_requests`<br>authenticated requester identity evidence ← `auth.permission_gate` | `owner_managed` | `cutover_ready` | field operations and finance | `docs/designs/WORK_ORDER_EXPENSE_ENTRY.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/runbooks/EXPENSE_CLAIM_ERP_CUTOVER.md`<br>`tests/test_field_expense_requests.py`<br>`tests/test_field_manager_api.py`<br>`tests/test_work_order_expense_web.py`<br>`tests/test_dotmac_erp_expense_sync.py`<br>`tests/playwright/e2e/test_work_order_expense_disclosure.py`<br>`tests/architecture/test_field_expense_request_writer_boundary.py`<br>`tests/architecture/test_field_request_history_identity.py`<br>`tests/integration/test_field_request_requester_history_migration.py` |
| `operations.expense_requests` | manager field expense review projection | `resolver` | canonical field expense request state ← `operations.expense_requests`<br>authorized manager expense review scope ← `auth.permission_gate`<br>canonical staff display identity ← `auth.staff_provisioning` | `owner_managed` | `cutover_ready` | field operations and finance | `docs/designs/WORK_ORDER_EXPENSE_ENTRY.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/runbooks/EXPENSE_CLAIM_ERP_CUTOVER.md`<br>`tests/test_field_expense_requests.py`<br>`tests/test_field_manager_api.py`<br>`tests/test_work_order_expense_web.py`<br>`tests/test_dotmac_erp_expense_sync.py`<br>`tests/playwright/e2e/test_work_order_expense_disclosure.py`<br>`tests/architecture/test_field_expense_request_writer_boundary.py`<br>`tests/architecture/test_field_request_history_identity.py`<br>`tests/integration/test_field_request_requester_history_migration.py` |
| `operations.expense_requests` | field expense requester-identity repair | `reconciler` | canonical field expense request state ← `operations.expense_requests`<br>authenticated requester and work-order access evidence ← `auth.permission_gate` | `owner_managed` | `cutover_ready` | field operations and finance | `docs/designs/WORK_ORDER_EXPENSE_ENTRY.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/runbooks/EXPENSE_CLAIM_ERP_CUTOVER.md`<br>`tests/test_field_expense_requests.py`<br>`tests/test_field_manager_api.py`<br>`tests/test_work_order_expense_web.py`<br>`tests/test_dotmac_erp_expense_sync.py`<br>`tests/playwright/e2e/test_work_order_expense_disclosure.py`<br>`tests/architecture/test_field_expense_request_writer_boundary.py`<br>`tests/architecture/test_field_request_history_identity.py`<br>`tests/integration/test_field_request_requester_history_migration.py` |
| `operations.material_dependencies` | contextual material need and ERP submission | `command_writer` | canonical service work-order state ← `operations.work_orders`<br>material dependency transition protocol ← `operations.work_order_status`<br>material-support cutover controls ← `control.settings_spec` | `owner_managed` | `cutover_ready` | field operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`docs/designs/MATERIALS_VENDOR_ERP_CHAIN.md`<br>`tests/test_field_material_requests.py`<br>`tests/test_dotmac_erp_material_sync.py`<br>`tests/test_admin_material_requests.py`<br>`tests/test_erp_material_webhook.py`<br>`field_mobile/test/materials_test.dart`<br>`tests/architecture/test_field_request_history_identity.py`<br>`tests/integration/test_field_request_requester_history_migration.py` |
...
| `operations.material_dependencies` | ERP-confirmed material request cancellation | `command_writer` | canonical material dependency state ← `operations.material_dependencies`<br>ERP material-support outcome observation ← `external:dotmac_erp`<br>material dependency transition protocol ← `operations.work_order_status` | `owner_managed` | `cutover_ready` | field operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`docs/designs/MATERIALS_VENDOR_ERP_CHAIN.md`<br>`tests/test_field_material_requests.py`<br>`tests/test_dotmac_erp_material_sync.py`<br>`tests/test_admin_material_requests.py`<br>`tests/test_erp_material_webhook.py`<br>`field_mobile/test/materials_test.dart`<br>`tests/architecture/test_field_request_history_identity.py`<br>`tests/integration/test_field_request_requester_history_migration.py` |
...
| `operations.material_dependencies` | requester-owned material request history | `resolver` | canonical material dependency state ← `operations.material_dependencies`<br>authenticated field requester identity ← `auth.permission_gate` | `owner_managed` | `cutover_ready` | field operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`docs/designs/MATERIALS_VENDOR_ERP_CHAIN.md`<br>`tests/test_field_material_requests.py`<br>`tests/test_dotmac_erp_material_sync.py`<br>`tests/test_admin_material_requests.py`<br>`tests/test_erp_material_webhook.py`<br>`field_mobile/test/materials_test.dart`<br>`tests/architecture/test_field_request_history_identity.py`<br>`tests/integration/test_field_request_requester_history_migration.py` |
| `operations.material_dependencies` | material request requester-identity repair | `reconciler` | canonical material dependency state ← `operations.material_dependencies`<br>authenticated field requester identity ← `auth.permission_gate` | `owner_managed` | `cutover_ready` | field operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`docs/designs/MATERIALS_VENDOR_ERP_CHAIN.md`<br>`tests/test_field_material_requests.py`<br>`tests/test_dotmac_erp_material_sync.py`<br>`tests/test_admin_material_requests.py`<br>`tests/test_erp_material_webhook.py`<br>`field_mobile/test/materials_test.dart`<br>`tests/architecture/test_field_request_history_identity.py`<br>`tests/integration/test_field_request_requester_history_migration.py` |
...
| `operations.project_lifecycle` | ProjectTemplate task-plan definition and application | `command_writer` | canonical project aggregate ← `operations.project_lifecycle`<br>authorized project command ← `auth.permission_gate` | `owner_managed` | `complete` | service delivery | `docs/designs/PROJECTS_SOT_COMPLETION.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_projects_service.py`<br>`tests/test_project_assignment_engine.py`<br>`tests/architecture/test_projects_sot_boundary.py` |
| `operations.project_lifecycle` | project customer-account eligibility | `policy` | canonical project aggregate ← `operations.project_lifecycle`<br>canonical customer account state ← `customer.accounts`<br>authorized project command ← `auth.permission_gate` | `owner_managed` | `complete` | service delivery | `docs/designs/PROJECTS_SOT_COMPLETION.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_projects_service.py`<br>`tests/test_project_assignment_engine.py`<br>`tests/architecture/test_projects_sot_boundary.py` |
...
| `operations.project_lifecycle` | project and task staff/team tag notification consequence | `event_policy` | canonical project aggregate ← `operations.project_lifecycle`<br>staff notification delivery queue ← `communications.notification_service` | `owner_managed` | `complete` | service delivery | `docs/designs/PROJECTS_SOT_COMPLETION.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_projects_service.py`<br>`tests/test_project_assignment_engine.py`<br>`tests/architecture/test_projects_sot_boundary.py` |
...
| `operations.installation_scope` | infrastructure-linked project installation scope creation | `command_writer` | canonical native project state ← `operations.project_lifecycle` | `participant` | `complete` | service delivery | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/SALES_TO_SERVICE_LIFECYCLE_SOT.md`<br>`docs/designs/PROJECT_INFRASTRUCTURE_SCOPE.md`<br>`tests/test_sales_to_service_lifecycle.py`<br>`tests/test_sales_orders_services.py`<br>`tests/test_sot_relationships.py` |
| `operations.installation_scope` | idempotent structural InstallationProject root creation | `command_writer` | canonical native project state ← `operations.project_lifecycle`<br>installation scope creation command ← `operations.project_lifecycle` | `participant` | `complete` | service delivery | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/SALES_TO_SERVICE_LIFECYCLE_SOT.md`<br>`docs/designs/PROJECT_INFRASTRUCTURE_SCOPE.md`<br>`tests/test_sales_to_service_lifecycle.py`<br>`tests/test_sales_orders_services.py`<br>`tests/test_sot_relationships.py` |
...
| `operations.vendor_purchase_invoices` | project-completion purchase-invoice origination | `application_coordinator` | canonical installation-project lifecycle state ← `operations.vendor_project_lifecycle`<br>canonical vendor purchase-invoice records ← `operations.vendor_purchase_invoice_records`<br>purchase-invoice currency policy ← `control.settings_spec`<br>purchase-invoice ERP tax-profile policy ← `control.settings_spec`<br>vendor project completion receipt protocol ← `events.owner_outputs` | `coordinator_managed` | `complete` | vendor operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/adr/0002-owner-command-transaction-boundary.md`<br>`tests/test_phase5_vendor_purchase_invoices.py`<br>`tests/test_vendor_payment_visibility.py`<br>`tests/test_vendor_submission_proposals.py`<br>`tests/architecture/test_materials_lifecycle_chain_boundary.py`<br>`tests/architecture/test_vendor_purchase_invoice_boundary.py` |
...
| `auth.staff_provisioning` | staff account provisioning | `application_coordinator` | ERP HR staff lifecycle request ← `external:dotmac_erp`<br>authorized RBAC assignment principal ← `auth.permission_gate`<br>active role catalog ← `auth.rbac_catalog`<br>managed role grant state ← `auth.system_user_assignments`<br>canonical staff identity and credential state ← `auth.staff_provisioning`<br>canonical Person Party identity ← `party.registry` | `coordinator_managed` | `complete` | platform security | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/ERP_WORKFORCE_ACCOUNT_PROVISIONING.md`<br>`docs/designs/STAFF_LOGIN_IDENTITY_RECONCILIATION.md`<br>`docs/adr/0002-owner-command-transaction-boundary.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`tests/test_api_staff_sync.py`<br>`tests/test_staff_provisioning_owner.py`<br>`tests/test_staff_login_identity_admin.py`<br>`tests/test_staff_login_identity_reconciliation_script.py`<br>`tests/architecture/test_staff_provisioning_boundary.py` |
| `auth.staff_provisioning` | staff identity bootstrap | `command_writer` | ERP HR staff lifecycle request ← `external:dotmac_erp`<br>canonical staff identity and credential state ← `auth.staff_provisioning`<br>canonical Person Party identity ← `party.registry` | `coordinator_managed` | `complete` | platform security | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/ERP_WORKFORCE_ACCOUNT_PROVISIONING.md`<br>`docs/designs/STAFF_LOGIN_IDENTITY_RECONCILIATION.md`<br>`docs/adr/0002-owner-command-transaction-boundary.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`tests/test_api_staff_sync.py`<br>`tests/test_staff_provisioning_owner.py`<br>`tests/test_staff_login_identity_admin.py`<br>`tests/test_staff_login_identity_reconciliation_script.py`<br>`tests/architecture/test_staff_provisioning_boundary.py` |
| `auth.staff_provisioning` | staff identity maintenance | `application_coordinator` | authorized staff identity principal ← `auth.permission_gate`<br>canonical staff identity and credential state ← `auth.staff_provisioning`<br>staff-linked field technician profile ← `auth.staff_provisioning` | `coordinator_managed` | `complete` | platform security | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/ERP_WORKFORCE_ACCOUNT_PROVISIONING.md`<br>`docs/designs/STAFF_LOGIN_IDENTITY_RECONCILIATION.md`<br>`docs/adr/0002-owner-command-transaction-boundary.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`tests/test_api_staff_sync.py`<br>`tests/test_staff_provisioning_owner.py`<br>`tests/test_staff_login_identity_admin.py`<br>`tests/test_staff_login_identity_reconciliation_script.py`<br>`tests/architecture/test_staff_provisioning_boundary.py` |
| `auth.staff_provisioning` | staff field technician profile binding | `command_writer` | authorized staff identity principal ← `auth.permission_gate`<br>canonical staff identity and credential state ← `auth.staff_provisioning`<br>staff-linked field technician profile ← `auth.staff_provisioning` | `coordinator_managed` | `complete` | platform security | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/ERP_WORKFORCE_ACCOUNT_PROVISIONING.md`<br>`docs/designs/STAFF_LOGIN_IDENTITY_RECONCILIATION.md`<br>`docs/adr/0002-owner-command-transaction-boundary.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`tests/test_api_staff_sync.py`<br>`tests/test_staff_provisioning_owner.py`<br>`tests/test_staff_login_identity_admin.py`<br>`tests/test_staff_login_identity_reconciliation_script.py`<br>`tests/architecture/test_staff_provisioning_boundary.py` |
| `auth.staff_provisioning` | staff display identity resolution | `resolver` | canonical staff identity and credential state ← `auth.staff_provisioning` | `coordinator_managed` | `complete` | platform security | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/ERP_WORKFORCE_ACCOUNT_PROVISIONING.md`<br>`docs/designs/STAFF_LOGIN_IDENTITY_RECONCILIATION.md`<br>`docs/adr/0002-owner-command-transaction-boundary.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`tests/test_api_staff_sync.py`<br>`tests/test_staff_provisioning_owner.py`<br>`tests/test_staff_login_identity_admin.py`<br>`tests/test_staff_login_identity_reconciliation_script.py`<br>`tests/architecture/test_staff_provisioning_boundary.py` |
| `auth.staff_provisioning` | staff login identity resolution | `resolver` | canonical staff identity and credential state ← `auth.staff_provisioning` | `coordinator_managed` | `complete` | platform security | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/ERP_WORKFORCE_ACCOUNT_PROVISIONING.md`<br>`docs/designs/STAFF_LOGIN_IDENTITY_RECONCILIATION.md`<br>`docs/adr/0002-owner-command-transaction-boundary.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`tests/test_api_staff_sync.py`<br>`tests/test_staff_provisioning_owner.py`<br>`tests/test_staff_login_identity_admin.py`<br>`tests/test_staff_login_identity_reconciliation_script.py`<br>`tests/architecture/test_staff_provisioning_boundary.py` |
| `auth.erp_staff_access` | ERP staff leave restriction projection | `projection_writer` | signed ERP staff leave restriction event ← `external:dotmac_erp`<br>canonical staff identity and credential state ← `auth.staff_provisioning` | `owner_managed` | `native` | platform security | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/ERP_STAFF_ACCESS_INTEGRATION.md`<br>`tests/test_erp_staff_access.py`<br>`tests/test_erp_staff_access_webhook.py`<br>`tests/test_erp_staff_access_reconciliation.py` |
| `auth.erp_staff_access` | ERP staff account-status projection | `projection_writer` | signed ERP staff account-status event ← `external:dotmac_erp`<br>canonical staff identity and credential state ← `auth.staff_provisioning` | `owner_managed` | `native` | platform security | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/ERP_STAFF_ACCESS_INTEGRATION.md`<br>`tests/test_erp_staff_access.py`<br>`tests/test_erp_staff_access_webhook.py`<br>`tests/test_erp_staff_access_reconciliation.py` |
| `auth.erp_staff_access` | ERP staff access reconciliation | `reconciler` | authoritative ERP staff access snapshot ← `external:dotmac_erp`<br>local ERP staff access projections ← `auth.erp_staff_access`<br>canonical staff identity and credential state ← `auth.staff_provisioning` | `owner_managed` | `native` | platform security | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/ERP_STAFF_ACCESS_INTEGRATION.md`<br>`tests/test_erp_staff_access.py`<br>`tests/test_erp_staff_access_webhook.py`<br>`tests/test_erp_staff_access_reconciliation.py` |
...
| `auth.oidc_mobile_federation` | field mobile OIDC ceremony lifecycle | `command_writer` | field mobile federation enablement ← `control.feature_registry`<br>field mobile federation configuration ← `control.settings_spec`<br>outstanding field mobile ceremony record ← `auth.oidc_mobile_federation` | `owner_managed` | `native` | platform security | `docs/designs/OIDC_MOBILE_FEDERATION.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_oidc_mobile_federation.py`<br>`tests/architecture/test_oidc_mobile_federation_boundary.py` |
| `auth.oidc_mobile_federation` | field mobile OIDC assertion admission | `command_writer` | field mobile federation enablement ← `control.feature_registry`<br>field mobile federation configuration ← `control.settings_spec`<br>outstanding field mobile ceremony record ← `auth.oidc_mobile_federation`<br>verified identity provider assertion ← `external:oidc_issuer`<br>installed OIDC verifier binding ← `party.credential_authentication_projection`<br>eligible staff principal ← `party.staff_authentication_reader` | `owner_managed` | `native` | platform security | `docs/designs/OIDC_MOBILE_FEDERATION.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_oidc_mobile_federation.py`<br>`tests/architecture/test_oidc_mobile_federation_boundary.py` |
...
| `access.pppoe_credentials` | canonical PPPoE access credential identity and secret | `authoritative_record` | canonical subscriber identity ← `customer.accounts`<br>canonical subscription lifecycle state ← `access.subscription_lifecycle`<br>catalog-linked target RADIUS profile ← `service_intent.catalog_policy`<br>PPPoE credential generation settings ← `control.settings_spec`<br>credential encryption policy ← `secrets.credential_crypto`<br>typed PPPoE credential command ← `access.pppoe_credentials` | `participant` | `shadowing` | network access | `docs/designs/PROVISIONING_LIFECYCLE_SOT.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_pppoe_auto_generation.py`<br>`tests/test_web_catalog_subscriptions.py`<br>`tests/architecture/test_pppoe_activation_boundary.py` |
| `access.pppoe_credentials` | subscription PPPoE login projection | `projection_writer` | canonical PPPoE credential record ← `access.pppoe_credentials`<br>canonical subscription lifecycle state ← `access.subscription_lifecycle` | `participant` | `shadowing` | network access | `docs/designs/PROVISIONING_LIFECYCLE_SOT.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_pppoe_auto_generation.py`<br>`tests/test_web_catalog_subscriptions.py`<br>`tests/architecture/test_pppoe_activation_boundary.py` |
| `access.credential_binding` | access credential subscription and RADIUS-profile binding | `command_writer` | canonical subscriber access credential ← `access.pppoe_credentials`<br>canonical subscription lifecycle state ← `access.subscription_lifecycle`<br>catalog-linked target RADIUS profile ← `service_intent.catalog_policy`<br>typed credential binding command evidence ← `access.credential_binding` | `participant` | `complete` | network access | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_subscription_correction.py`<br>`tests/architecture/test_subscription_correction_boundary.py` |
...
| `service_intent.offer_access_requirement` | access-classified offer-version admission | `authoritative_record` | explicit access-requirement classification ← `service_intent.offer_access_requirement`<br>canonical offer identity ← `service_intent.catalog_policy` | `owner_managed` | `native` | commercial operations | `docs/designs/CATALOG_ACCESS_REQUIREMENT_AUTHORITY.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_offer_access_requirement.py`<br>`tests/architecture/test_offer_access_requirement_boundary.py` |
| `service_intent.offer_access_requirement` | immutable access requirement for an exact offer version | `authoritative_record` | canonical offer-version record ← `service_intent.offer_access_requirement` | `owner_managed` | `native` | commercial operations | `docs/designs/CATALOG_ACCESS_REQUIREMENT_AUTHORITY.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_offer_access_requirement.py`<br>`tests/architecture/test_offer_access_requirement_boundary.py` |
| `service_intent.offer_access_requirement` | reviewed classification of legacy/unclassified versions | `command_writer` | authenticated reviewed classification command ← `auth.permission_gate`<br>canonical offer-version record ← `service_intent.offer_access_requirement` | `owner_managed` | `native` | commercial operations | `docs/designs/CATALOG_ACCESS_REQUIREMENT_AUTHORITY.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_offer_access_requirement.py`<br>`tests/architecture/test_offer_access_requirement_boundary.py` |
| `service_intent.offer_access_requirement` | mutation of an already-admitted offer version | `command_writer` | authenticated offer-version mutation command ← `auth.permission_gate`<br>canonical offer-version record ← `service_intent.offer_access_requirement` | `owner_managed` | `native` | commercial operations | `docs/designs/CATALOG_ACCESS_REQUIREMENT_AUTHORITY.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_offer_access_requirement.py`<br>`tests/architecture/test_offer_access_requirement_boundary.py` |
...
| `integration.installations` | payment webhook ingress traffic policy | `command_writer` | integration installation protocol ← `integration.installations`<br>canonical integration installation aggregate ← `integration.installations` | `owner_managed` | `complete` | platform integrations | `docs/designs/INTEGRATION_PLATFORM_SOT.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_integration_installations.py`<br>`tests/test_integration_installation_api.py`<br>`tests/test_integration_meta_social.py`<br>`tests/test_team_inbox_whatsapp_webhook.py`<br>`tests/test_integration_manifest_deployment_gate.py`<br>`tests/architecture/test_integration_platform_boundary.py` |
...
| `integration.meta_lead_conversion` | Meta customer-conversion delivery projection | `projection_writer` | committed Lead account conversion ← `events.store`<br>verified existing-customer match ← `sales.meta_lead_customer_match`<br>immutable Meta Lead origin ← `sales.lead_lifecycle`<br>enabled Meta conversion capability ← `integration.installations` | `owner_managed` | `native` | sales and platform integrations | `docs/designs/INTEGRATION_PLATFORM_SOT.md`<br>`docs/designs/MARKETING_SALES_SOT.md`<br>`tests/test_meta_lead_conversion.py` |
| `integration.meta_lead_conversion` | Meta customer-conversion delivery lifecycle | `command_writer` | Meta conversion delivery protocol ← `integration.meta_lead_conversion`<br>Meta conversion transport receipt ← `integration.runtime` | `owner_managed` | `native` | sales and platform integrations | `docs/designs/INTEGRATION_PLATFORM_SOT.md`<br>`docs/designs/MARKETING_SALES_SOT.md`<br>`tests/test_meta_lead_conversion.py` |
...
| `integration.workforce_attendance_adapter` | field location-sharing attendance eligibility | `policy` | ERP attendance observation ← `external:dotmac_erp` | `read_only` | `native` | workforce integrations | `docs/designs/WORKFORCE_ATTENDANCE_INTEGRATION.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_workforce_attendance_capability.py`<br>`tests/test_admin_dashboard_attendance.py`<br>`tests/test_field_attendance_api.py`<br>`field_mobile/test/attendance_test.dart`<br>`tests/architecture/test_integration_platform_boundary.py` |
...
| `integration.dotmac_erp_operational_context_adapter` | per-domain ERP operational-context delivery watermarks and retry admission | `projection_writer` | canonical Sub projects and project tasks ← `operations.project_lifecycle`<br>canonical Sub support tickets ← `support.ticket_lifecycle`<br>canonical Sub service work orders ← `operations.work_order_commands`<br>ERP version-2 operational-sync response ← `external:dotmac_erp` | `owner_managed` | `cutover_ready` | service delivery integrations | `docs/designs/CONFIGURABLE_ERP_OPERATIONAL_SYNC.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_dotmac_erp_domain_sync.py` |
| `integration.procurement_purchase_order_cutover` | Selfcare procurement ERP ownership cutover and reconciled PO backfill | `application_coordinator` | canonical approved vendor quote targets ← `operations.vendor_project_records`<br>fresh verified ERP supplier bindings ← `external:dotmac_erp`<br>procurement single-writer controls ← `control.settings_spec`<br>procurement cutover command evidence ← `integration.procurement_purchase_order_cutover` | `coordinator_managed` | `cutover_ready` | vendor finance integrations | `docs/designs/MATERIALS_VENDOR_ERP_CHAIN.md`<br>`docs/runbooks/PURCHASE_ORDER_ERP_CUTOVER.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_procurement_purchase_order_cutover.py`<br>`tests/architecture/test_procurement_purchase_order_cutover_boundary.py` |
| `integration.dotmac_erp_payables_adapter` | Dotmac ERP purchase-invoice payload mapping | `projection_writer` | canonical vendor purchase-invoice records ← `operations.vendor_purchase_invoice_records`<br>ERP purchase-invoice origination response ← `external:dotmac_erp`<br>ERP purchase-invoice flow controls ← `control.settings_spec`<br>ERP purchase-invoice tax-profile control ← `control.settings_spec` | `owner_managed` | `cut_over` | vendor finance integrations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`tests/test_vendor_payment_visibility.py`<br>`tests/test_dotmac_erp_outbox.py` |
...
| `integration.dotmac_erp_material_support_adapter` | Sub-to-Dotmac-ERP material-support payload mapping | `resolver` | approved canonical material dependency ← `operations.material_dependencies`<br>ERP material-support transport contract ← `control.settings_spec` | `coordinator_managed` | `cutover_ready` | field operations integrations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/designs/SOT_CODING_STANDARDS_REFACTOR.md`<br>`docs/designs/MATERIALS_VENDOR_ERP_CHAIN.md`<br>`docs/runbooks/MATERIAL_REQUEST_ERP_CUTOVER.md`<br>`tests/test_dotmac_erp_material_sync.py`<br>`tests/test_erp_material_webhook.py`<br>`tests/test_field_material_requests.py` |
...
| `ui.ticket_sla_report` | current ticket SLA operational summary | `resolver` | typed ticket SLA report query ← `ui.ticket_sla_report`<br>authorized support-report scope ← `auth.permission_gate`<br>canonical current Ticket lifecycle state ← `support.ticket_lifecycle`<br>current ticket SLA records ← `support.ticket_sla_clock`<br>canonical service-team identity ← `operations.service_team_lifecycle` | `read_only` | `complete` | Self-Care reporting | `docs/designs/CRM_REPORT_DATA_FLOW_GUIDE.md`<br>`docs/designs/OPERATIONS_MEASUREMENT_STRATEGY.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_ticket_sla_reports.py`<br>`tests/architecture/test_ticket_sla_report_boundary.py` |
| `ui.ticket_sla_report` | ticket SLA violation queue and export projection | `resolver` | typed ticket SLA report query ← `ui.ticket_sla_report`<br>authorized support-report scope ← `auth.permission_gate`<br>canonical current Ticket lifecycle state ← `support.ticket_lifecycle`<br>current ticket SLA records ← `support.ticket_sla_clock`<br>canonical service-team identity ← `operations.service_team_lifecycle` | `read_only` | `complete` | Self-Care reporting | `docs/designs/CRM_REPORT_DATA_FLOW_GUIDE.md`<br>`docs/designs/OPERATIONS_MEASUREMENT_STRATEGY.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_ticket_sla_reports.py`<br>`tests/architecture/test_ticket_sla_report_boundary.py` |
| `ui.ticket_sla_report` | ticket SLA historical clock trend projection | `resolver` | typed ticket SLA report query ← `ui.ticket_sla_report`<br>authorized support-report scope ← `auth.permission_gate`<br>canonical current Ticket lifecycle state ← `support.ticket_lifecycle`<br>current ticket SLA records ← `support.ticket_sla_clock` | `read_only` | `complete` | Self-Care reporting | `docs/designs/CRM_REPORT_DATA_FLOW_GUIDE.md`<br>`docs/designs/OPERATIONS_MEASUREMENT_STRATEGY.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_ticket_sla_reports.py`<br>`tests/architecture/test_ticket_sla_report_boundary.py` |
...
| `ui.customer_list_projection` | admin customer complete CSV scope and analytical projection | `resolver` | normalized customer list query ← `ui.list_contracts`<br>canonical visible customer accounts ← `customer.accounts`<br>canonical subscription lifecycle records ← `access.subscription_lifecycle`<br>canonical billing-mode profile ← `financial.billing_profile`<br>effective non-standard billing treatment ← `financial.subscription_billing_treatments`<br>canonical recurring catalog price ← `service_intent.catalog_policy`<br>canonical catalog offers ← `service_intent.catalog_policy`<br>canonical network access identities ← `network.identity`<br>canonical service IP assignments ← `network.ip_assignment_lifecycle`<br>canonical support ticket lifecycle records ← `support.ticket_lifecycle`<br>canonical customer payment records ← `financial.payments`<br>canonical customer invoice records ← `financial.invoices` | `read_only` | `complete` | subscriber operations UI | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/FRONTEND_SPEC.md`<br>`tests/test_web_customer_lists.py`<br>`tests/test_customer_list_ui_contract.py`<br>`tests/test_customer_export.py`<br>`tests/test_sot_relationships.py` |
...
| `ui.field_live_map_projection` | dispatch field-map sharing-authorized technician position projection | `resolver` | native field-technician presence facts ← `ui.field_live_map_projection` | `read_only` | `complete` | field operations UI | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/designs/FIELD_MANAGER_TEAM_MAP.md`<br>`tests/test_admin_maps_web.py`<br>`field_mobile/test/manager_team_map_test.dart`<br>`tests/architecture/test_field_live_map_boundary.py` |
| `ui.field_live_map_projection` | dispatch field-map searchable fields and focus coordinates | `resolver` | native field-technician presence facts ← `ui.field_live_map_projection`<br>canonical work-order map facts ← `operations.work_orders`<br>canonical subscriber service-address facts ← `customer.accounts`<br>dispatch field-map search input ← `ui.field_live_map_projection` | `read_only` | `complete` | field operations UI | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/designs/FIELD_MANAGER_TEAM_MAP.md`<br>`tests/test_admin_maps_web.py`<br>`field_mobile/test/manager_team_map_test.dart`<br>`tests/architecture/test_field_live_map_boundary.py` |
| `ui.field_live_map_projection` | dispatch field-map stale-position semantics | `policy` | native field-technician presence facts ← `ui.field_live_map_projection`<br>dispatch field-map freshness input ← `ui.field_live_map_projection` | `read_only` | `complete` | field operations UI | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/designs/FIELD_MANAGER_TEAM_MAP.md`<br>`tests/test_admin_maps_web.py`<br>`field_mobile/test/manager_team_map_test.dart`<br>`tests/architecture/test_field_live_map_boundary.py` |
| `ui.field_live_map_projection` | selected technician nearest-address projection | `resolver` | native field-technician presence facts ← `ui.field_live_map_projection`<br>provider-neutral reverse-geocode result ← `gis.geocoding`<br>dispatch field-map freshness input ← `ui.field_live_map_projection` | `read_only` | `complete` | field operations UI | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/designs/FIELD_MANAGER_TEAM_MAP.md`<br>`tests/test_admin_maps_web.py`<br>`field_mobile/test/manager_team_map_test.dart`<br>`tests/architecture/test_field_live_map_boundary.py` |
...
| `ui.work_order_expense_projection` | admin work-order expense-entry form projection | `resolver` | canonical work-order expense scope ← `operations.work_orders`<br>authenticated requester scope ← `auth.permission_gate`<br>ERP expense category observation ← `operations.expense_categories`<br>UI projection vocabulary ← `ui.projection_contracts` | `read_only` | `complete` | field operations and finance UI | `docs/designs/WORK_ORDER_EXPENSE_ENTRY.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_work_order_expense_web.py`<br>`tests/test_dispatch_work_orders_csrf.py`<br>`tests/test_admin_route_permissions.py` |
| `ui.work_order_expense_projection` | requester-owned work-order expense status projection | `resolver` | canonical work-order expense scope ← `operations.work_orders`<br>authenticated requester scope ← `auth.permission_gate`<br>canonical expense request and ERP delivery evidence ← `operations.expense_requests` | `read_only` | `complete` | field operations and finance UI | `docs/designs/WORK_ORDER_EXPENSE_ENTRY.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_work_order_expense_web.py`<br>`tests/test_dispatch_work_orders_csrf.py`<br>`tests/test_admin_route_permissions.py` |
| `ui.work_order_expense_projection` | work-order expense form validation policy | `policy` | ERP expense category observation ← `operations.expense_categories`<br>typed expense form input ← `ui.work_order_expense_projection` | `read_only` | `complete` | field operations and finance UI | `docs/designs/WORK_ORDER_EXPENSE_ENTRY.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_work_order_expense_web.py`<br>`tests/test_dispatch_work_orders_csrf.py`<br>`tests/test_admin_route_permissions.py` |
...
| `ui.project_list_projection` | admin project-form customer picker projection | `resolver` | project authoring customer search request ← `ui.project_list_projection`<br>canonical customer account identity ← `customer.accounts`<br>authorized project authoring access ← `auth.permission_gate` | `read_only` | `complete` | service delivery UI | `docs/designs/PROJECTS_SOT_COMPLETION.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_customer_detail_navigation.py`<br>`tests/test_web_projects_service.py`<br>`tests/test_web_admin_projects_render.py`<br>`tests/test_web_dispatch_work_orders.py`<br>`tests/test_projects_api.py`<br>`tests/architecture/test_projects_sot_boundary.py` |
...
| `ui.quote_detail_projection` | admin Quote delivery eligibility and activity presentation | `resolver` | canonical Quote detail state ← `sales.service`<br>canonical Quote audit evidence ← `observability.audit_log`<br>canonical Quote delivery outcome ← `communications.notification_service`<br>canonical Quote payment-review decision ← `sales.quote_payment_review` | `read_only` | `native` | sales operations UI | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`tests/test_quote_documents_and_delivery.py`<br>`tests/test_quote_payment_review.py`<br>`tests/architecture/test_quote_document_delivery_boundary.py` |
...
| `ui.action_readiness_contracts` | action readiness state and blocker vocabulary | `policy` | action readiness contract vocabulary ← `ui.action_readiness_contracts` | `not_applicable` | `inventoried` | platform UI | `docs/designs/ACTION_READINESS_CONTRACT.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_action_readiness.py`<br>`tests/architecture/test_action_readiness_ownership.py` |
| `ui.action_readiness_contracts` | blocker owner attribution invariant | `policy` | action readiness contract vocabulary ← `ui.action_readiness_contracts` | `not_applicable` | `inventoried` | platform UI | `docs/designs/ACTION_READINESS_CONTRACT.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_action_readiness.py`<br>`tests/architecture/test_action_readiness_ownership.py` |
| `ui.action_readiness_contracts` | readiness correlation and operation reference contract | `policy` | action readiness contract vocabulary ← `ui.action_readiness_contracts` | `not_applicable` | `inventoried` | platform UI | `docs/designs/ACTION_READINESS_CONTRACT.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_action_readiness.py`<br>`tests/architecture/test_action_readiness_ownership.py` |
| `ui.action_readiness_contracts` | readiness-derived action gating contract | `policy` | action readiness contract vocabulary ← `ui.action_readiness_contracts` | `not_applicable` | `inventoried` | platform UI | `docs/designs/ACTION_READINESS_CONTRACT.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_action_readiness.py`<br>`tests/architecture/test_action_readiness_ownership.py` |
...
| `sales.crm_quote_retirement` | retired CRM quote execution and action availability | `policy` | approved CRM retirement ← `sales.crm_quote_retirement` | `not_applicable` | `complete` | sales operations | `docs/designs/CRM_WEB_RETIREMENT.md`<br>`tests/test_quotes_mirror.py`<br>`tests/architecture/test_no_crm_writeback.py` |
...
| `sales.marketing_conversion_projection` | PII-free immutable Fiber conversion milestone projection | `projection_writer` | immutable captured origin evidence ← `sales.capture`<br>committed customer lifecycle event ← `events.dispatcher` | `owner_managed` | `complete` | sales operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/PARTY_CUSTOMER_LIFECYCLE.md`<br>`docs/designs/MARKETING_SALES_SOT.md`<br>`tests/test_marketing_conversion_projection.py`<br>`tests/test_fiber_inquiry_webhook.py`<br>`tests/architecture/test_sot_registry_liveness.py` |
| `sales.meta_lead_customer_match` | Meta Lead customer-match projection | `projection_writer` | captured Meta Lead contact observations ← `sales.capture`<br>verified customer contact identity ← `party.registry` | `owner_managed` | `native` | sales operations | `docs/designs/MARKETING_SALES_SOT.md`<br>`docs/designs/SALES_TO_SERVICE_LIFECYCLE_SOT.md`<br>`tests/test_meta_lead_ads.py` |
...
| `sales.quote_payment_review` | request customer Quote payment review | `command_writer` | typed payment-review request ← `sales.quote_payment_review`<br>canonical Quote commercial state ← `sales.service` | `owner_managed` | `native` | sales operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/designs/SALES_TO_SERVICE_LIFECYCLE_SOT.md`<br>`tests/test_quote_payment_review.py`<br>`tests/test_customer_quote_payments.py`<br>`mobile/test/quote_model_test.dart` |
| `sales.quote_payment_review` | staff approval of customer Quote payment | `command_writer` | typed payment-review command ← `sales.quote_payment_review`<br>canonical Quote commercial state ← `sales.service`<br>authorized staff reviewer ← `auth.permission_gate` | `owner_managed` | `native` | sales operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/designs/SALES_TO_SERVICE_LIFECYCLE_SOT.md`<br>`tests/test_quote_payment_review.py`<br>`tests/test_customer_quote_payments.py`<br>`mobile/test/quote_model_test.dart` |
| `sales.quote_payment_eligibility` | authenticated customer Quote payment eligibility and payable amount | `resolver` | authorized customer Quote payment scope ← `sales.quote_payment_eligibility`<br>canonical Quote commercial state ← `sales.service`<br>canonical Quote payment-review state ← `sales.quote_payment_review`<br>canonical Quote deposit settlement state ← `financial.invoices`<br>installation-backed Paystack availability ← `financial.payment_routing` | `read_only` | `native` | sales and finance operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/designs/QUOTATION_PDF_PAYMENT_OPTIONS.md`<br>`docs/designs/PAYMENT_GATEWAY_CONTROL_PLANE_SOT.md`<br>`tests/test_customer_quote_payments.py`<br>`tests/test_quote_documents_and_delivery.py`<br>`tests/architecture/test_quote_document_delivery_boundary.py` |
| `sales.quote_delivery` | idempotent branded Quote email request | `command_writer` | Quote delivery command evidence ← `sales.quote_delivery`<br>canonical Quote commercial state ← `sales.service`<br>canonical Party recipient state ← `party.registry`<br>canonical customer account recipient state ← `customer.accounts`<br>canonical Quote PDF artifact ← `sales.quote_documents`<br>canonical Subscriber Quote payment eligibility ← `sales.quote_payment_eligibility` | `owner_managed` | `native` | sales operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/UI_INFORMATION_AND_ACTION_STANDARD.md`<br>`docs/designs/QUOTATION_PDF_PAYMENT_OPTIONS.md`<br>`tests/test_quote_documents_and_delivery.py`<br>`tests/architecture/test_quote_document_delivery_boundary.py` |
...
3. `financial.tax_configuration` owns configurable tax-rate records and their
   ...
   Recurring billing, prepaid renewal, and the explicit subscription
   first-invoice action all obtain their typed rate identity and application
   from `financial.billing_tax_resolution`; no adapter derives or omits VAT.
...
19. `integration.inbox` owns signature-verified payment receipt identity,
   ...
   can precede activation; the live receipt route remains
   enabled-only, and missing, invalid, quarantined, retired, or unconfigured
...
21. `financial.account_credit_deposits` owns the typed Deposit Account Credit
   ...
   enters its typed owner-managed settlement command; payment-reconciliation,
   ...
   projection, and `financial.payment_reconciliation` is its repair owner. That
   owner uses a work-conserving bounded batch with reserved capacity for both stale
   pending work and terminal late-success recovery whenever both lanes are due;
   the configured batch minimum is two. Supported providers interleave within each
   lane starting with the least recently served provider. Every successfully
   claimed intent durably advances its typed attempt progress before provider I/O;
   a candidate that loses a concurrent claim performs no provider I/O. A transport
   or consequence error therefore cannot pin the oldest row. Its backlog projection
   partitions pending work into
   fresh, due, cooling-down, and outside-window states and terminal recovery into
   due, cooling-down, and outside-window states, with lifecycle-lane counts,
   oldest due age, and selected/checked plus durable attempt progress as explicit
   drift evidence. The automatic maximum-age policy is not a retention bypass.
   `financial.payment_reconciliation` separately owns a read-only, fresh-provider
   preview/classification and a fingerprint-bound finance-reviewed recovery for
   one exact outside-window Paystack intent/reference whose canonical status is
   failed, abandoned, canceled, or expired. Pending and other non-terminal rows
   are ineligible for this operator path even outside the automatic window.
   Confirmation rechecks the exact intent, canonical active PaymentProvider from
   `financial.payment_gateway_finance`, version-pinned reconcile binding,
   normalized transaction identity and money facts, and canonical
   Payment/provider-event replay evidence. Its single owner transaction atomically
   composes the same canonical settlement and lifecycle participants with immutable,
   append-only
   recovery-run evidence carrying actor, reason, evidence/change reference,
   fingerprints, idempotency and result lineage. A newly created Payment uses
   current owner confirmation as `paid_at`, with any customer-subledger
   `occurred_at` derived from it; exact link/replay preserves the existing
   canonical Payment date. The command never backdates to intent creation or an
   unauthenticated provider timestamp. It never widens the scheduled window,
   accepts a cohort, adds a second provider-event trust class, or posts money
   directly.
...
- Prepaid settlement calendar boundary: a prepaid settlement begins its paid
  service period after the exact uninterrupted entitlement or applied service-
  extension coverage containing the payment instant. If no exact coverage
  exists, a lapsed replacement begins on the payment's `Africa/Lagos` calendar
  date, not its UTC date. `financial.prepaid_service_renewals` resolves the
  boundary through the typed cadence and returns UTC instants for persistence.
...
- Billing-anchor writer boundary: `Subscription.next_billing_at` is a projection
  ...
  - `funding_observation` (payment creation, allocation, refund, reversal) may
    ...
    `financial.subscription_billing_grants` grant, or another still-unresolved
    projection. Exact service-extension coverage is already part of the floor;
    payment re-anchoring does not infer or preserve a second extension delta.
    Advancement is monotonic while the invoice's own entitlements survive.
...
7. `customer.service_status` owns customer-visible service health and action
   ...
   the authoritative amount required by financial policy. It consumes the
   lifecycle-owned, typed operationally-current cohort at one explicit `as_of`
   instant: only past-ended `disabled` or `stopped` rows become historical for
   customer health. Account Health receives that exact cohort, while admin
   history retains every subscription. A deterministic fingerprinted drift
   report identifies all non-terminal rows whose explicit end has passed and
   fails closed on missing or ambiguous replacement chronology; lifecycle
...
9. `customer.reseller_status_actions` (`app/services/reseller_portal.py`) owns
   ...
   The companion `customer.reseller_ticket_projection` owns the reseller-
   scoped native Support count and account-ticket list. It validates canonical
   reseller/account ownership and consumes `support.ticket_lifecycle` records;
   retired CRM capability bindings are never queried or used as a fallback.
...
15b. Field requester expense history is resolved by
    ...
    Person Party, and every historically linked technician profile. An absent,
    inactive, or replaced technician profile cannot hide an exactly owned
    claim, while unrelated identities remain excluded. The filtered total is
    computed before pagination. Manager-technicians receive both the personal
    requester projection and the separate approval queue. The approval queue
    resolves each requester's display label through `auth.staff_provisioning`;
    the UI does not reinterpret identity, ownership, or approval eligibility.
...
9. Staff notification service owns internal/admin notification creation,
   ...
   requests into the assigned admin notification inbox. Ticket and project
   owners submit typed assignment requests containing the source command,
   entity UUID, recipient UUIDs, and an exact internal detail target. The staff
   notification owner rejects dashboard-only or external assignment targets and
   materializes one source-linked personal inbox row per recipient. For payment proofs it
   ...
   when it verifies or rejects the proof.
   `communications.staff_notification_read_state` owns the personal menu and
   unread-count query plus the atomic read-state transition. Opening an inbox
   item is scoped to its assigned system user and follows only its stored target;
   the target action performs its own domain permission check again.
...
4. `operations.work_order_commands`: owns native work-order creation and header
   ...
   assignment-queue transitions. Staff/team tag tokens on work orders stage
   in-app notifications through `communications.staff_notifications`; ordinary
   descriptive tags remain local work-order metadata.
...
7. `operations.material_dependencies`: owns the material need and approval that
   ...
   workflow. ERP-backed cancellation remains `cancellation_pending` until ERP
   confirms refusal; an observed issue wins a concurrent cancellation race.
...
9. `operations.vendor_project_lifecycle` (`app.services.vendor_portal_operations`)
   ...
   adapters around this owner. Project completion is the automatic payables
   determinant for PO-backed vendor work: a receipted consumer may create one
   system-approved vendor purchase invoice from the approved quote and enqueue
   the ERP invoice against the existing ERP purchase order. Project
   verification is still an operational decision: invoice approval and ERP
   payment observations do not gate it and are not modified by it. The owner
   raises transport-neutral
...
10. `operations.vendor_purchase_invoices` owns vendor purchase-invoice state,
   ...
   It also consumes `vendor_project.completed` as the automatic trigger for
   PO-backed ERP invoicing: when a completed project has an approved quote and
   no active vendor invoice, it creates one system-approved invoice from that
   quote and enqueues the payables export through the same durable ERP outbox.
   Existing draft or submitted vendor invoices are not overwritten. The ERP
   purchase-invoice payload carries the configured
   `vendor_purchase_invoice_erp_tax_profile`, allowing ERP to apply the same
   tax profile operators select when creating an invoice from a PO manually.
...
6. `sales.quote_payment_review`: owns staff approval or rejection of the exact
   customer Quote snapshot, including reviewer, time, revision, audit, and
...
9. `referrals.program`: owns Party-first capture policy, canonical ReferralCode,
...
10. `referrals.account_conversion`: owns exact Referral/Party/Lead context
