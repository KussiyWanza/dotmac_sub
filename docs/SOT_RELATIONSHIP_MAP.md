...
| `billing.opening_balance_history` | carried-source identity classification | `resolver` | canonical migrated customer identity ← `customer.accounts` | `read_only` | `cutover_ready` | billing and finance operations | `docs/adr/0007-end-to-end-billing-target-architecture.md`<br>`docs/designs/SPLYNX_RETIREMENT.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_opening_balance_history.py`<br>`tests/test_billing_alignment_audit.py`<br>`tests/test_subledger_opening_positions.py` |
