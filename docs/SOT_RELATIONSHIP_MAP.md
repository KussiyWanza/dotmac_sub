...
| `network.fiber_cost_items` | fiber drop-cost components and their prices | `command_writer` | operator-priced fiber cost components ← `network.fiber_cost_items` | `owner_managed` | `native` | network operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_fiber_cost_items.py` |
| `network.fiber_cost_items` | whether a drop estimate can be produced, and what it totals | `resolver` | operator-priced fiber cost components ← `network.fiber_cost_items` | `owner_managed` | `native` | network operations | `docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_fiber_cost_items.py` |
