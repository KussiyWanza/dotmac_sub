...
| `operations.expense_requests` | field expense request submission | `command_writer` | canonical service work-order state ← `operations.work_orders` | `owner_managed` | `cutover_ready` | field operations and finance | `docs/SOT_RELATIONSHIP_MAP.md`<br>`docs/runbooks/EXPENSE_CLAIM_ERP_CUTOVER.md`<br>`tests/test_field_expense_requests.py`<br>`tests/test_dotmac_erp_expense_sync.py` |
