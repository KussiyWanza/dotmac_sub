...
persistence-like mutation must name a declared owner. The 221 existing
...
| `party.staff_session_projection` | approved staff session Party projection | `projection_writer` | approved staff session projection decision ← `party.staff_session_projection`<br>canonical staff principal state ← `auth.staff_provisioning`<br>canonical Person Party identity ← `party.registry`<br>legacy staff session state ← `app_sessions.auth` | `owner_managed` | `shadowing` | identity and authentication | `docs/PARTY_PRINCIPAL_CONTEXT_BINDING.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/test_staff_session_party_adoption.py` |
