...
| `network.outage_work_order_handoff` | shared-outage infrastructure ticket issuance | `application_coordinator` | locked outage incident ← `network.outage_lifecycle`<br>outage-response routing team ← `operations.service_team_composition`<br>typed infrastructure ticket create input ← `network.outage_work_order_handoff`<br>canonical infrastructure ticket ← `support.ticket_lifecycle` | `coordinator_managed` | `native` | network operations | `docs/designs/OUTAGE_WORK_ORDER_HANDOFF_SOT.md`<br>`docs/SOT_RELATIONSHIP_MAP.md`<br>`tests/services/topology/test_outage_work_order_handoff_contract.py`<br>`tests/architecture/test_ticket_work_order_handoff_boundary.py` |
