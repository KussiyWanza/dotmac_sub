...
| Response cohorts | **Resolved.** Unreplied and Needs Attention are separate, transaction-current projections. Needs Attention requires a customer message, a successful human-agent reply, and a later customer follow-up with no later successful agent reply; it excludes resolved, snoozed, inactive, ticketed, and Meta comment conversations. |
