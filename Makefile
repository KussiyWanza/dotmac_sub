...
prod-restart: ## Recreate prod app + worker services from the current image (APP_IMAGE)
	$(PROD_COMPOSE) up -d app celery-worker celery-worker-bandwidth celery-worker-ingestion celery-worker-monitoring celery-worker-notifications celery-worker-billing celery-worker-tr069 celery-beat bandwidth-poller syslog-listener
